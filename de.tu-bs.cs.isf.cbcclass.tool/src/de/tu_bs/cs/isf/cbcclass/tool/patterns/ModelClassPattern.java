...
public class ModelClassPattern extends IdPattern implements IPattern {
	...
	public Object[] create(ICreateContext context) {
		...
		while (inheritance.equals("")) {
			inheritance = (String) JOptionPane.showInputDialog(null,
					"Should this class inherit? Type a class name or press cancel.");
			if (inheritance != null)
				inheritanceResource = ClassUtil.getClassModelResource(path, inheritance.trim());
	...
	protected boolean layout(IdLayoutContext context, String id) {
		...
		if (id.equals(ID_NAME_TEXT)) {
			...
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionText + sizeText * getIndex(ga),
					mainRectangle.getWidth(), sizeText);
			changesDone = true;
		} else if (id.equals(ID_CLASS_FIELDS_TEXT)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, beginYFieldHeader
					+ sizeText * (sizeInvariants == 0 ? getIndex(ga) + 1 : getIndex(ga) - sizeInvariants + 1) - 7,
					mainRectangle.getWidth(), sizeText);
			changesDone = true;
			...
			polyline.getPoints().clear();
			List<Point> pointList = Graphiti.getGaService()
					.createPointList(new int[]{0, beginYFieldHeader, mainRectangle.getWidth(), beginYFieldHeader});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
			...
			List<Point> pointList = Graphiti.getGaService().createPointList(
					new int[]{0, beginYFieldHeader + 20, mainRectangle.getWidth(), beginYFieldHeader + 20});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
		}
		return changesDone;
	...
	protected IReason updateNeeded(IdUpdateContext context, String id) {
		if (id.equals(ID_MAIN_RECTANGLE)) {
			...
			if (mc.getInheritsFrom() != null) {
				var inheritee = (ModelClass) ClassUtil
						.getClassModelResource(FileUtil.getProjectLocation(mc.getInheritsFrom().eResource().getURI()),
				...
				for (int j = 0; j < objects.size(); j++) {
					...
					if (obj instanceof Text) {
						...
						for (int k = 0; k < invs.size(); k++) {
							...
							if (("{" + c.getName() + "}").equalsIgnoreCase(((Text) obj).getValue())
									&& !found.contains(index)) {
								found.add(index);
								break;
							}
						...
						for (int k = 0; k < inheritedInvs.size(); k++) {
							...
							if (("{" + c.getName() + "}").equalsIgnoreCase(
									((Text) obj).getValue().replace(" inherited", "")) && !found.contains(index)) {
								found.add(index);
								break;
							}
						}
	...
	protected boolean update(IdUpdateContext context, String id) {
		if (id.equals(ID_NAME_TEXT)) {
			...
			List<Field> inheritedFields = new ArrayList<Field>();
			if (modelClass.getInheritsFrom() != null) {
				var inheritee = (ModelClass) ClassUtil.getClassModelResource(
						FileUtil.getProjectLocation(modelClass.getInheritsFrom().eResource().getURI()),
						modelClass.getInheritsFrom().getName()).getContents().get(0);
			...
			for (int i = 0; i < invs.size() + inheritedInvs.size(); i++) {
				...
				for (int j = numShapesOfEmptyClass; j < shapes.size(); j++) {
					...
					if (shape.getGraphicsAlgorithm() instanceof TextImpl) {
						...
						if (inv.getName()
								.equals(text.getValue().replace(" inherited", "").replace("{", "").replace("}", ""))) {
							check = true;
							checkedShapes.add(j);
							break;
						}
					} else {
						if (!checkedShapes.contains(j))
							checkedShapes.add(j);
					}
				}
				if (!check) {
			...
			for (int i = 0; i < fields.size() + inheritedFields.size(); i++) {
				Field field = i < inheritedFields.size()
						? inheritedFields.get(i)
						: fields.get(i - inheritedFields.size());
				EList<Shape> shapes = ((ContainerShape) context.getPictogramElement()).getChildren();
				boolean check = false;
				for (int j = numShapesOfEmptyClass; j < shapes.size(); j++) {
					Shape shape = shapes.get(j);
