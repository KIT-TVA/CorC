 ...
 *
 */
public class VerifyPreImplPre extends MyAbstractAsynchronousCustomFeature {
	 ...
	 *            The FeatureProvider
	 */
	public VerifyPreImplPre(IFeatureProvider fp) {
		super(fp);
	}

	@Override
	public String getName() {
		return "Verify the preconditions";
	}

	@Override
	public String getDescription() {
		return "Verifies that the precondition of the parent implies the precondition of the child.";
	}
	...
	public void execute(ICustomContext context, IProgressMonitor monitor) {
		monitor.beginTask("Verify pre->pre", IProgressMonitor.UNKNOWN);
		PictogramElement[] pes = context.getPictogramElements();
		if (pes != null && pes.length == 2) {
			Object first = getBusinessObjectForPictogramElement(pes[0]);
			Object second = getBusinessObjectForPictogramElement(pes[1]);
