...
public class ParameterDescription {
	...
	public ParameterDescription(Object object, DynamicParameter annotation, Parameterized parameterized,
			ResourceBundle bundle, JCommander jc) {
		if (!Map.class.isAssignableFrom(parameterized.getType())) {
			throw new ParameterException("@DynamicParameter " + parameterized.getName() + " should be of type "
	...
	public ParameterDescription(Object object, Parameter annotation, Parameterized parameterized, ResourceBundle bundle,
			JCommander jc) {
	...
	private ResourceBundle findResourceBundle(Object o) {
		...
		if (p != null && !isEmpty(p.resourceBundle())) {
			...
			src.com.beust.jcommander.ResourceBundle a = o.getClass()
					.getAnnotation(src.com.beust.jcommander.ResourceBundle.class);
	...
	private void initDescription(String description, String descriptionKey, String[] names) {
		...
		if (!"".equals(descriptionKey)) {
			if (m_bundle != null) {
				...
				// field.getName()
				// + " has a descriptionKey but no bundle was defined with @ResourceBundle,
				// using " +
		...
		for (String name : names) {
			if (name.length() > m_longestName.length())
				m_longestName = name;
		}
	}
	...
	public static void validateParameter(ParameterDescription pd, Class<? extends IParameterValidator> validator,
	...
	/*
	 * Tests if its the first time a non-default value is being added to the field.
	 */
