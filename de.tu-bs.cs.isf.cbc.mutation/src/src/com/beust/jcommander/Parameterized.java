...
public class Parameterized {
		...
		while (!Object.class.equals(cls)) {
			for (Field f : cls.getDeclaredFields()) {
				...
				if (annotation != null) {
					...
					result.add(new Parameterized(new WrappedParameter((DynamicParameter) dynamicParameter), null, f,
							null));
	...
	public Object get(Object object) {
		try {
			if (m_method != null) {
				if (m_getter == null) {
					m_getter = m_method.getDeclaringClass().getMethod("g" + m_method.getName().substring(1),
							new Class[0]);
				}
