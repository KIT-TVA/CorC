...
import src.com.beust.jcommander.internal.Console;
import src.com.beust.jcommander.internal.DefaultConsole;
import src.com.beust.jcommander.internal.DefaultConverterFactory;
import src.com.beust.jcommander.internal.JDK6Console;
...
public class JCommander {
		...
		if (m_mainParameterDescription != null) {
			if (m_mainParameterDescription.getParameter().required() && !m_mainParameterDescription.isAssigned()) {
				throw new ParameterException(
						"Main parameters are required (\"" + m_mainParameterDescription.getDescription() + "\")");
			}
		}
	}
	...
	private void addDescription(Object object) {
		...
		// }
		// p("Adding description for " + name);
		// ParameterDescription pd = new ParameterDescription(object, dp, f, m_bundle,
		// this);
		// m_fields.put(f, pd);
		// m_descriptions.put(name, pd);
		//
	...
	private int processFixedArity(String[] args, int originalIndex, ParameterDescription pd, Class<?> fieldType,
			int arity) {
		int index = originalIndex;
		String arg = args[index];
		// Special case for boolean parameters of arity 0
		if (arity == 0 && (Boolean.class.isAssignableFrom(fieldType) || boolean.class.isAssignableFrom(fieldType))) {
			pd.addValue("true");
			m_requiredFields.remove(pd.getParameterized());
	...
	/**
	 * Store the help for the command in the passed string builder, indenting every
	 * line with "indent".
	 */
	...
	private Object convertToList(String value, IStringConverter<?> converter,
			Class<? extends IParameterSplitter> splitterClass) throws InstantiationException, IllegalAccessException {
			...
			if (!alias.equals(name)) {
				...
				if (mappedName != null && !mappedName.equals(progName)) {
					throw new ParameterException("Cannot set alias " + alias + " for " + name
							+ " command because it has already been defined for " + mappedName.m_name + " command");
				}
