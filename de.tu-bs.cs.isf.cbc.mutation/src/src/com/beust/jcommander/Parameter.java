...
public @interface Parameter {
	...
	/**
	 * An array of allowed command line parameters (e.g. "-d", "--outputdir",
	 * etc...). If this attribute is omitted, the field it's annotating will receive
	 * all the unparsed options. There can only be at most one such annotation.
	 */
	...
	/**
	 * How many parameter values this parameter will consume. For example, an arity
	...
	/**
	 * The string converter to use for this field. If the field is of type
	 * <tt>List</tt> and not <tt>listConverter</tt> attribute was specified,
	 * JCommander will split the input in individual values and convert each of them
	 * separately.
	 */
	Class<? extends IStringConverter<?>> converter() default NoConverter.class;

	/**
	 * The list string converter to use for this field. If it's specified, the field
	 * has to be of type <tt>List</tt> and the converter needs to return a List
	 * that's compatible with that type.
	 */
	...
	/**
	 * What splitter to use (applicable only on fields of type <tt>List</tt>). By
	 * default, a comma separated splitter will be used.
	 */
	Class<? extends IParameterSplitter> splitter() default CommaParameterSplitter.class;

	/**
	 * If true, console will not echo typed input Used in conjunction with password
	 * = true
	 */
