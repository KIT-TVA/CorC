...
public class ETC extends src.mujava.op.util.Mutator {
	public ETC(FileEnvironment file_env, ClassDeclaration cdecl, CompilationUnit comp_unit) {
	...
	void genETC(ThrowStatement p) {
		try {
			...
			if (expr instanceof AllocationExpression) {
				...
				InheritanceINFO inf = src.mujava.MutationSystem.getInheritanceInfo(exp_type);
				if (inf == null)
					return;
