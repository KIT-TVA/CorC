...
public class SDL extends MethodLevelMutator {
	...
	public void generateWhileMutants(Statement statement) {
		...
		for (int j = 0; j < whileStatementList.size(); j++) {
			...
			if (!isVariableDeclaration(whileStatementList.get(j)) && !isReturnStatement(whileStatementList.get(j))
					&& !isTryStatement(whileStatementList.get(j)) && !isEmptyStatement(whileStatementList.get(j))) {
	...
	public void generateIfMutants(Statement statement) {
		...
		for (int j = 0; j < ifStatementList.size(); j++) {
			...
			if (!isVariableDeclaration(ifStatementList.get(j)) && !isReturnStatement(ifStatementList.get(j))
					&& !isTryStatement(ifStatementList.get(j)) && !isEmptyStatement(ifStatementList.get(j))) {
				ifMutant.remove(j);
		...
		for (int j = 0; j < elseStatementList.size(); j++) {
			...
			if (!isVariableDeclaration(elseStatementList.get(j)) && !isReturnStatement(elseStatementList.get(j))
					&& !isTryStatement(elseStatementList.get(j)) && !isEmptyStatement(elseStatementList.get(j))) {
				elseMutant.remove(j);
	...
	public void generateForMutants(Statement statement) {
		...
		for (int j = 0; j < forStatementList.size(); j++) {
			...
			if (!isVariableDeclaration(forStatementList.get(j)) && !isReturnStatement(forStatementList.get(j))
					&& !isTryStatement(forStatementList.get(j)) && !isEmptyStatement(forStatementList.get(j))) {
				forMutant.remove(j);
				outputToFile(forStatementList, forMutant);
			}
		}

		// ForStatement forMutantStatement = new ForStatement(forStatement.getInit(),
		// forStatement.getCondition(), forStatement.getIncrement(),
		...
		// ForStatement forMutantStatement = new ForStatement(forStatement.getInit(),
		// null, forStatement.getIncrement(), forStatement.getStatements());
	...
	public void generateTryMutants(Statement statement) {
		...
		for (int i = 0; i < catchList.size(); i++) {
			...
			for (int j = 0; j < catchListStatementList.size(); j++) {
				...
				outputToFile(tryStatement, tryMutant);
				// TryStatement mTryStatement = new TryStatement(tryStatement.getBody(),
				// catchList);
