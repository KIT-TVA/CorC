	...
	public void visit(openjava.ptree.MethodDeclaration m) throws ParseTreeException {
		...
		for (String line : lines) {
			if (line.contains("requires")) {
				...
				{/////////// Weaken ==
					// A.length > 0 && (\forall int i; i>=0 && i<A.length; A[i] , 0 || A[i] , 1 ||
					// A[i] , 2); [size=4]
				...
				{/////////// Weaken >
					...
					for (int j = 1; j < parts.length; ++j) {
						...
						// Create mutants
						String mutant1 = genNewContract(lines, prefix + ">=" + postfix, i).replaceAll("##", ">=")
								.replaceAll("IMP", "==>");
						String mutant2 = genNewContract(lines, prefix + "!=" + postfix, i).replaceAll("##", ">=")
								.replaceAll("IMP", "==>");
				...
				/*
				 * requires A.length > 0 && (\forall int i; i>=0 && i<A.length; A[i] == 0 ||
				 * A[i] == 1 || A[i] == 2);
				 */
				...
				{/////////// Weaken &&
					...
					/*
					 * requires A.length > 0, (\forall int i; i>=0, i<A.length; A[i] == 0 || A[i] ==
					 * 1 || A[i] == 2)
					 */
					for (int j = 1; j < parts.length; ++j) {
						String prefix = "";
						...
						// postfix: j=1: (\forall int i; i>=0 && i<A.length; A[i] == 0 || A[i] == 1 ||
						// A[i] == 2);
				...
				{/////////// Weaken forall
					...
					/*
					 * requires A.length > 0(\, int i; i>=0, i<A.length; A[i] == 0 || A[i] == 1 ||
					 * A[i] == 2)
					 */
					for (int j = 1; j < parts.length; ++j) {
						String prefix = "";
				...
				{/////////// Weaken P<=Q 
					String[] parts = line.replaceAll("<==>", "BIIMP").replaceAll(" <= ", "<=").replaceAll("<=", " <= ")
							.replaceAll("  ", " ").split(" ");
					...
					for (int j = 0; j < parts.length; ++j) {
						...
						if (tmp[j].equals("<=")) {
							...
							// Create mutants
							String mutant1 = genNewContract(lines, String.join(" ", tmp).replaceAll("BIIMP", "<==>"),
									i);
