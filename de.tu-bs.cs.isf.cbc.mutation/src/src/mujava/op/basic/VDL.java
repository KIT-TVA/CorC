...
public class VDL extends Arithmetic_OP {
	...
	private void aorMutantGen(BinaryExpression exp) {
		...
		if (mutantLeft instanceof Variable || mutantLeft instanceof ArrayAccess) // if left is variable or an array
																					// access
		{// System.out.println("b "+exp);
			aor_outputToFile(exp, mutantRight); // delete it, only keep right
		}
		if (mutantRight instanceof Variable || mutantRight instanceof ArrayAccess) // if right is variable or an array
																					// access
		{// System.out.println("b "+exp);
