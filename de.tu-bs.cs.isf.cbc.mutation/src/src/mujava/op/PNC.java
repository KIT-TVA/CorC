...
public class PNC extends src.mujava.op.util.Mutator {
	...
	boolean isOverridingMethod(OJMethod parent, Method child) {
		...
		if (((parent_pars == null) || (parent_pars.length == 0)) && ((child_pars == null) || (child_pars.length == 0)))
			return true;
	...
	/*
	 * --- Old Version // --------------------------------- private boolean
	 * pncPossible(String p, String c,ExpressionList arg_list){ try{ OJClass clazz;
	 * OJClass parent_class; clazz = OJClass.forName(c);
	 * if(clazz.getName().equals(p)) return false; while(
	 * !(clazz.getName().equals("java.lang.Object")) ){ parent_class =
	 * clazz.getSuperclass(); if(parent_class.getName().equals(p)){ int arg_num =
	 * arg_list.size(); if(arg_num>0){ OJClass[] types = new OJClass[arg_num];
	 * for(int j=0;j<arg_num;j++){ types[j] = getType(arg_list.get(j)); }
	 * if(clazz.getDeclaredConstructor(types)!=null) return true; }else{ // default
	 * constructor : There is no constructor declared
	 * if(clazz.getConstructors().length==0) return true; // There is one
	 * constrcutor which has no arguments if(clazz.getConstructor(null)!=null)
	 * return true; } } clazz = parent_class; } return false; }catch(Exception e){
	 * return false; }catch(Error er){ return false; } }
	 */
