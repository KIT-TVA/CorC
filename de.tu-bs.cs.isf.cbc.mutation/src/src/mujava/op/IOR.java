...
/**
 * <p>
 * Generate IOR (Overriding method rename) mutants -- rename the parent's
 * versions of methods that are overridden in a subclass so that the overriding
 * does not affect the parents' method
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
public class IOR extends src.mujava.op.util.Mutator {
	Stack s;
		...
		try {
			...
			for (int i = 0; i < parent_method.length; i++) {
				...
				for (int j = 0; j < size; j++) {
					// System.out.print(parent_par[j].getName()+" : " +
					// child_parList.get(j).getTypeSpecifier().getName());
