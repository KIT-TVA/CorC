...
/**
 * <p>
 * Generate ABS (Absolute Value Insertion), AOR (Arithmetic Operator
 * Replacement), LCR (Logical Operator Replacement), ROR (Relational Operator
 * Replacement), and UOI (Unary Operator Insertion) mutants.
 * </p>
 * <p>
 * -- <i>ABS</i> modifies each arithmetic expression by the function
 * <i>abs()</i>, <i>negAbs()</i>, and <i>failOnZero()</i> <br />
 ...
 * (bitwise and - &, bitwise or - |, exclusive or - ^) by each of the other
 * operators <br />
 * -- <i>ROR</i> replaces each occurrence of one of the relational operators
 * (<, <=, >, >=, =, <>) by each of the other operators and by <i>falseOp</i>
 * which assigns a <i>false</i> value to a predicate, and <i>trueOp</i> which
 * assigns a <i>true</i> value to a predicate <br />
 * -- <i>UOI</i> inserts each unary operator (arithmetic +, arithmetic -,
 * conditional !, logical ~) before each expression of the correct type
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
public class ABS_AOR_LCR_ROR_UOI extends src.mujava.op.util.Mutator {
