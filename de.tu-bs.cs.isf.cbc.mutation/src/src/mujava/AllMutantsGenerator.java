...
import src.mujava.op.AMC;
import src.mujava.op.EAM;
import src.mujava.op.EMM;
import src.mujava.op.EOA;
import src.mujava.op.EOC;
import src.mujava.op.IHD;
import src.mujava.op.IHI;
import src.mujava.op.IOD;
import src.mujava.op.IOP;
import src.mujava.op.IOR;
import src.mujava.op.IPC;
import src.mujava.op.ISD;
import src.mujava.op.JDC;
import src.mujava.op.JID;
import src.mujava.op.JSD;
import src.mujava.op.JSI;
import src.mujava.op.JTD;
import src.mujava.op.JTI;
import src.mujava.op.OAN;
import src.mujava.op.OMD;
import src.mujava.op.OMR;
import src.mujava.op.PCC;
import src.mujava.op.PCD;
import src.mujava.op.PCI;
import src.mujava.op.PMD;
import src.mujava.op.PNC;
import src.mujava.op.PPD;
import src.mujava.op.PRV;
...
import src.mujava.op.basic.PS;
import src.mujava.op.basic.PW;
	...
	void genClassMutants1(ClassDeclarationList cdecls) {
		for (int j = 0; j < cdecls.size(); ++j) {
			...
			if (cdecl.getName().equals(MutationSystem.CLASS_NAME)) {
				...
				try {
					src.mujava.op.util.Mutator mutant_op;
		...
		for (int j = 0; j < cdecls.size(); ++j) {
			...
			if (cdecl.getName().equals(MutationSystem.CLASS_NAME)) {
				try {
					...
					if (hasOperator(traditionalOp, "PW")) {
						Debug.println("  Applying PW ... ... ");
						mutant_op = new PW(file_env, cdecl, comp_unit);
						comp_unit.accept(mutant_op);
					}
					if (hasOperator(traditionalOp, "PS")) {
						Debug.println("  Applying PS ... ... ");
						mutant_op = new PS(file_env, cdecl, comp_unit);
						comp_unit.accept(mutant_op);
					}
				} catch (ParseTreeException e) {
