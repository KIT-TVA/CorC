...
public class TestExecuter {
	...
	private TestResult runMutants(TestResult tr, String methodSignature)
							...
							try {
								...
								for (int k = 0; k < testCases.length; k++) {
									...
									for (Annotation annotation : annotations) {
										...
										// + annotation.toString().indexOf("@org.junit.Test"));
										if (annotation.toString().indexOf("@org.junit.Test") != -1) {
											// killed_mutants[k]= ""; // At first, no mutants are killed by each test
											// case
								...
								for (Failure failure : listOfFailure) {
									 ...
									 * testSourceName.length() + 5); System.out.println("tempLineNumber: " +
									 * tempLineNumber); lineNumber = tempLineNumber.substring(0,
									 * tempLineNumber.indexOf(")")); //System.out.print("LineNumber: " +
									 * lineNumber); }
									 */
									// get the test name that has the error and save the failure info to the results
									// for mutants
									if (failure.getMessage() == null)
