...
/**
 * <p>
 * Template for viewing mutants: show original source code, source code of the
 * selected mutants, summary of mutants
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
public abstract class MutantsViewerPanel extends JPanel {
	...
	public void showMutant(String dir_name, int changed_line) {
		...
		try {
			...
			int caret_pos = 0;
			String strLine;
			File myFile = new File(MutationSystem.MUTANT_PATH + "/" + dir_name, MutationSystem.CLASS_NAME + ".java");
	...
	int getMutatedLineNum(String str) {
		...
		// (Upsorn, 07/14/2009) Modify end_index. Originally, a line of log is formatted
		// as
		// mutant_name:line_no:original_string => mutated_string
		// In html, it is possible to have <A
		// Href="http://www.ise.gmu.edu/~ofut/classes/632/">
		...
		System.out.println("MutantsViewerPanel.getMutatedLineNum --- " + "start_index =" + start_index
				+ " : end_index =" + end_index);
	...
	String getMutatedContent(String str) {
		...
		log_str = log_str.substring(start_index + 1);
		int end_index = start_index + 1 + log_str.indexOf(MutationSystem.LOG_IDENTIFIER);
		System.out.println("MutantsViewerPanel.getMutatedContent --- " + "start_index =" + start_index
				+ " : end_index =" + end_index);
