...
import src.mujava.ClassMutantsGenerator;
...
public class MutantsGenPanel extends JPanel {
	...
	protected void initColumnSizes(JTable table, AbstractTableModel model) {
		 ...
		 * column.setMaxWidth(30); break; case 1 : column.setMaxWidth(90); break; case 2
		 * : column.setMaxWidth(80); break; } }
		 */
	}
	...
	void runB_mouseClicked(MouseEvent e) {
		// check if any files are selected, return an error message if no files have
		// been selected
		...
		// check if any operators are selected, return an error message if no files have
		// been selected
		...
		for (int i = 0; i < file_list.length; i++) {
			...
			try {
				...
				// Added on 1/19/2013, no mutants will be generated for a class having only one
				// main method
				...
				// System.out.println(" -- Can't apply because " + file_name+ " is 'abstract'
				// class ");
				// break;
				// case MutationSystem.APPLET :
				// System.out.println(" -- Can't apply because " + file_name+ " is 'applet'
				// class ");
				// break;
				...
				// System.out.println("Can't generate mutants for " +file_name + " because
				// OpenJava " + oje.getMessage());
				deleteDirectory();
			} catch (Exception exp) {
				System.out.println("[Exception] " + file_name + " " + exp.toString());
				exp.printStackTrace();
				// System.out.println("Can't generate mutants for " +file_name + " due to
				// exception" + exp.getClass().getName());
				// exp.printStackTrace();
				deleteDirectory();
			} catch (Error er) {
				...
				// System.out.println("Can't generate mutants for " +file_name + " due to error"
				// + er.getClass().getName());
				deleteDirectory();
			}
		}
