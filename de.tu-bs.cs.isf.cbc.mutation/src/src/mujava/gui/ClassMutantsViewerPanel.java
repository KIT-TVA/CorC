...
/**
 * <p>
 * Template for viewing class-level mutants: show original source code, source
 * code of the selected mutants, summary of mutants
 * </p>
 * 
 * @author Yu-Seung Ma
 * @version 1.0
...
public class ClassMutantsViewerPanel extends MutantsViewerPanel {
	...
	void jbInit() throws Exception {
		...
		/**
		 * summary table: containing the numbers of mutants by each class-level mutation
		 * operators along with the total number of mutants generated
		 */
		...
		/**
		 * show a list of mutants to be selected for viewing -- click on the mutant to
		 * display the original source and mutated code
		 */
	...
	void printGeneratedMutantNum(String[] operators, int[] num) {
		 ...
		 * ���� for(int i=0; i<operators.length; i++){ // num �迭 �ʱ�ȭ num[i] = 0; }
		 * if(name!=null){ for(int i=0;i<name.length;i++){ for(int j=0;
		 * j<operators.length;j++){ // ==> if(name[i].indexOf(operators[j]+"_")==0){
		 * num[j]++;} // ==> } } }
		 */
		// ȭ�鿡 �����ִ� �ҽ�
		int total = 0;
