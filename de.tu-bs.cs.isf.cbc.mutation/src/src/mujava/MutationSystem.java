...
/**
 * <p>
 * Description: Control an entire MuJava system -- specify path of MuJava
 * system; specify path of Java source files which mutation is applied to;
 * specify path of classes of Java source files; specify path which mutants,
 * class mutants, traditional mutants, and exception-related mutants are put
 * into; specify path where test cases are located; list names of class mutation
 * operators, traditional mutation operators, and exception-related mutation
 * operators; and clean up previously generated mutants
 * </p>
 * 
 * <p>
 ...
 * </p>
 * <p>
 * Limitation: MuJava currently does not support interface, abstract, gui, and
 * applet.
 * 
 * @author Yu-Seung Ma
 * @update by Nan Li May 2012
 * @new function addURL: classes are added to the CLASSPATH dynamically so that
 *      users do not need to set up the CLASSPATH manually any more
 * @version 1.0
 */
...
public class MutationSystem extends OJSystem {
	 ...
	 * "COR","COD","COI","SOR","LOR","LOI","LOD" ,"SCR","SCI","SCD"};
	 */

	/** List of names of traditional mutation operators */
	// Upsorn: (04/06/2009) added Statement Deletion operator (SID, SWD, SFD, SSD)
	public static String[] tm_operators = {"AORB", "AORS", "AOIU", "AOIS", "AODU", "AODS", "ROR", "COR", "COD", "COI",
			"SOR", "LOR", "LOI", "LOD", "ASRS", "SDL", "VDL", "CDL", "ODL"};
	// "IBD", "WBD", "FBD", "SBD" };
	// "SID", "SWD", "SFD", "SSD" };
	// "SDL"};

	/** List of names of exception-related mutation operators */
	public static String[] em_operators = {"EFD", "EHC", "EHD", "EHI", "ETC", "ETD"};
	...
	private static boolean isGUI(Class c) {
		while (c != null) {
			if ((c.getName().indexOf("java.awt") == 0) || (c.getName().indexOf("javax.swing") == 0))
				return true;
	...
	/**
	 * Examine if type is of primitive types (boolean, byte, char, short, int, long,
	 * double, void)
	 */
	...
	public static void recordInheritanceRelation() throws Exception {
		...
		if (classes == null) {
			...
			System.err.println(" Please check the directory  " + MutationSystem.CLASS_PATH
	 ...
	 * 
	 * @param name
	 *            of class (including package name)
	 */
	public static void setJMutationPaths(String whole_class_name) {
		...
		MutationSystem.ORIGINAL_PATH = MutationSystem.MUTANT_HOME + "/" + whole_class_name + "/"
				+ MutationSystem.ORIGINAL_DIR_NAME;
		MutationSystem.CLASS_MUTANT_PATH = MutationSystem.MUTANT_HOME + "/" + whole_class_name + "/"
				+ MutationSystem.CM_DIR_NAME;
		MutationSystem.TRADITIONAL_MUTANT_PATH = MutationSystem.MUTANT_HOME + "/" + whole_class_name + "/"
				+ MutationSystem.TM_DIR_NAME;
		MutationSystem.EXCEPTION_MUTANT_PATH = MutationSystem.MUTANT_HOME + "/" + whole_class_name + "/"
	 ...
	 * </p>
	 * <p>
	 * ** CAUTION : this function or `setJMutationStructure(String home_path)'
	 * should be called before generating and running mutants.
	 */
	...
	/**
	 * <p>
	 * Recognize file structure for mutation system from not "mujava.config" but
	 * from user directly
	 * </p>
	 */
