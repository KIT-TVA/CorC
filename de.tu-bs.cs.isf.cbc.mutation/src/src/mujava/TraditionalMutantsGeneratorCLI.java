...
/**
 * <p>
 * Description: New tranditional mutants generator class build exclusively for
...
public class TraditionalMutantsGeneratorCLI extends TraditionalMutantsGenerator {
	...
	/**
	 * Verify if the target Java source and class files exist, generate traditional
	 * mutants
	 */
	void genMutants() {
		if (comp_unit == null) {
	...
	/**
	 * Apply selected traditional mutation operators: AORB, AORS, AODU, AODS, AOIU,
	 * AOIS, ROR, COR, COD, COI, SOR, LOR, LOI, LOD, ASRS, SID, SWD, SFD, SSD
	 * 
	 * @param cdecls
	 */
		...
		for (int j = 0; j < cdecls.size(); ++j) {
			...
			if (cdecl.getName().equals(MutationSystem.CLASS_NAME)) {
				try {
					 ...
					 * Debug.println("  Applying SID ... ... "); mutant_op = new SID(file_env,
					 * cdecl, comp_unit); comp_unit.accept(mutant_op); }
					 * 
					 * if (hasOperator (traditionalOp, "SWD") ) {
					 * Debug.println("  Applying SWD ... ... "); mutant_op = new SWD(file_env,
					 * cdecl, comp_unit); comp_unit.accept(mutant_op); }
					 */
