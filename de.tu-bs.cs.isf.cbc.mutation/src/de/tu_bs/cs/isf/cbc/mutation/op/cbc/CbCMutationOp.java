...
public abstract class CbCMutationOp {
	...
	protected void getOperatorPosition(List<String> ops, String cStr) {
		for (String o : ops) {
			...
			if (isPartialOp(cStr, this.opToMutateStart)) continue;
	...
	protected String[] mutate(List<String> ops, String cStr) {
		...
		for (String o : ops) {
			...
			this.mutations[i] = cStr.substring(0, this.opToMutateStart) 
					+ handleOp(o)
	...
	private boolean isPartialOp(String s, int pos) {
		...
		if (pos > 0 && !Character.isJavaIdentifierPart(s.charAt(pos-1)) && !Arrays.asList(' ', '(', ')', '[', ']').contains(s.charAt(pos-1))) {
			return true;
		}
		if (pos < s.length() - 1 && !Character.isJavaIdentifierPart(s.charAt(pos+1)) && !Arrays.asList(' ', '(', ')', '[', ']').contains(s.charAt(pos+1))) {
			return true;
		}
		return false;
	}
