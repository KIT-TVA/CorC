	...
	public static File preProcess(File f) {
		...
		try {
			...
			for (int i = 0; i < lines.size(); i++) {
				...
				// Filtering pure, save original in a map with old to be replaced with contains
				// in postProcessing
				...
				// Filtering strictly_pure, save original in a map with old to be replaced with
				// contains in postProcessing
				// Assumption: Method Signature does not get changed through Mutation!
				...
				// Filtering spec_public non_null, save original in a map with old to be
				// replaced with contains in postProcessing
				// Assumption: Method Signature does not get changed through Mutation!
				...
				// Filtering nullable, save original in a map with old to be replaced with
				// contains in postProcessing
				// Assumption: Method Signature does not get changed through Mutation!
		...
		try {
			...
			for (int i = 0; i < lines.size(); i++) {
				...
				if (lines.get(i).contains("public static  void abcdefghijklmnopqrstuvwxyz")) {
					...
					// Replace marked pure, strictly_pure, spec_public non null, or nullable methods
					// with the signature part containing pure comment
					...
					procLines.add(
							pureHelperMapping.get(lines.get(i).replaceAll("[ \t\b\f\r\n]", "") + "{").replace("{", ""));
					...
					procLines.add(
							nullableMapping.get(lines.get(i).replaceAll("[ \t\b\f\r\n]", "") + "{").replace("{", ""));
				} else {
					procLines.add(lines.get(i));
				}
			}
