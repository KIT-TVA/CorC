...
public class RenamingPattern extends IdPattern implements IPattern {
	...
	protected boolean layout(IdLayoutContext context, String id) {
		...
		if (renaming.getRename().size() >= 1) {
			height = height / (renaming.getRename().size() + 1);
		}
	...
	protected IReason updateNeeded(IdUpdateContext context, String id) {
		if (id.equals(ID_MAIN_RECTANGLE)) {
			...
			if (containerShape.getChildren().size() - 1 != renaming.getRename().size() * 4) {
	...
	protected boolean update(IdUpdateContext context, String id) {
		if (id.equals(ID_MAIN_RECTANGLE)) {
			...
			while ((((ContainerShape) context.getPictogramElement()).getChildren().size() - 1) / 4 < renames.size()) {
				int newIndex = (((ContainerShape) context.getPictogramElement()).getChildren().size() - 1) / 4;
