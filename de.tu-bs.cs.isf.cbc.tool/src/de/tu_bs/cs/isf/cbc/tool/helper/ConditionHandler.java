...
public final class ConditionHandler {
	...
	while (start != -1) {
	    condition = condition.substring(0, start) + returnVar.getName().split("\\s")[1]
	...
	while (m.find()) {
	    ...
	    c = c.substring(0, m.start()) + "IntStream.of(" + params[0].trim() + ").boxed().toList().subList("
		    + params[1].strip() + ", " + params[2].strip() + ").stream().max(Integer::compare).get() == "
		    + params[0].trim() + "[" + params[3].trim() + "]" + c.substring(m.end(), c.length());
	    m = p.matcher(c);
	}
	...
	while (m.find()) {
	    final String innerPart = m.group(0).substring(m.group(0).indexOf("(") + 1, m.group(0).indexOf(")"));
	    final String[] params = innerPart.split("\\,");
	    c = c.substring(0, m.start()) + "IntStream.of(" + params[0].strip() + ")" + ".boxed().toList().subList("
		    + params[2].strip() + ", " + params[3].strip() + ").contains(" + params[1].strip() + ")"
    ...
    public static String translateCondition(final URI projectPath, final String condition, String instanceName,
	    List<InputData> gVars, boolean replaceMathFuncs) {
	String c = condition;
	...
	if (!instanceName.isEmpty()) {
	    ...
	    var className = Character.toUpperCase(instanceName.charAt(0))
		    + instanceName.substring(1, instanceName.length());
    ...
    public static String translateCondition(final URI projectPath, final String condition, String instanceName,
	    List<InputData> gVars) {
	return translateCondition(projectPath, condition, instanceName, gVars, true);
    }

    public static Node parseCondition(final URI projectPath, final String condition, String instanceName,
	    List<InputData> gVars) throws UnexpectedTokenException {
    ...
    public static String addInstanceNameToVar(String condition, final String instanceName,
	    final List<InputData> globalVars) {
	for (var globalVar : globalVars) {
	    Pattern p = Pattern.compile("^" + Pattern.quote(globalVar.getName()) + "\\W|\\W"
		    + Pattern.quote(globalVar.getName()) + "$|" + "\\W" + Pattern.quote(globalVar.getName()) + "\\W");
	    Matcher m = p.matcher(condition);
	    int start = -1;

	    while (m.find()) {
		if (condition.charAt(m.start()) == '.')
		    continue;
		if (m.start() == 0) {
		    ...
		    condition = condition.substring(0, start) + instanceName + "."
			    + m.group(0).substring(start, m.end() - 1)
			    + condition.substring(m.end() - 1, condition.length());
		} else {
		    if (condition.substring(0, m.start() + 1).endsWith("old("))
			continue;
		    condition = condition.substring(0, m.start() + 1) + instanceName + "."
			    + condition.substring(m.start() + 1, m.end() - 1)
			    + condition.substring(m.end() - 1, condition.length());
		}
    ...
    public static String addInstanceNameToMethod(String condition, final String instanceName,
	    final List<String> methodLst) {
	...
	for (var method : methods) {
		...
		if (startIndex == 0 || condition.charAt(startIndex - 1) != '.') {
		    ...
		    if (startIndex > 0) {
			if (Character.isAlphabetic(condition.charAt(startIndex - 1)))
			    continue;
		    }
		    if (m.end() + 1 < condition.length()) {
			if (Character.isAlphabetic(condition.charAt(m.end() + 1)))
			    continue;
		    }
		    var prevLen = condition.length();
		    condition = condition.substring(0, startIndex) + instanceName + "." + m.group(0)
			    + condition.substring(m.end(), condition.length());
    ...
    public static String initializeOldVars(final String postCondition, String classVarStr, int numTabs,
	    List<String> paramNames) {
	// TODO: old keyword can be used for none instance variables as well like for
	// parameters for example. (example diagram: transfer)
	String output = "";
	var tabs = "";
