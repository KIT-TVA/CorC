...
public class VerifyOriginalCallStatementProofGraphBegin extends MyAbstractAsynchronousCustomFeature {
	...
	public void execute(ICustomContext context, IProgressMonitor monitor) {
		...
		VerifyStatementProofGraphComplete verifyProofGraph = new VerifyStatementProofGraphComplete(
				getFeatureProvider());
			...
			if (bo instanceof AbstractStatement statement) {
				...
				try {
					...
					for (List<String> path : pathsToFork) {
						...
						genCode.generateWithRestriction(project.getLocation(), callingFeature, callingClass,
								callingMethod, featureConfig, path.get(path.size() - 1));
						...
						ProveWithKey prove = new ProveWithKey(statement, getDiagram(), monitor, fileHandler,
								featureConfig, 0,
								path.size() == 1
										? KeYInteraction.ABSTRACT_PROOF_BEGIN
										: KeYInteraction.ABSTRACT_T_RESOLVED_PROOF);
						boolean proven = prove.proveStatementWithKey(null, refinements, refinementVars, false, false,
