...
public class VerifyAllStatements extends MyAbstractAsynchronousCustomFeature {
	...
	public void execute(ICustomContext context, IProgressMonitor monitor) {
		...
		if (isVariational) {
			...
			featureConfigs = VerifyFeatures.verifyConfig(uri, callingMethod, true, callingClass, false, null);
			...
			if (featureConfigsRelevant != null) {
				...
				if (CompareMethodBodies.readAndTestMethodBodyWithJaMoPP2(statement.getName())) {
					ProveWithKey prove = new ProveWithKey(statement, diagram, monitor,
							new FileUtil(uRi.toPlatformString(true)), featureConfigs[configNum], configNum,
							KeYInteraction.ABSTRACT_PROOF_FULL);
					List<CbCFormula> refinements = verifyStmt.generateCbCFormulasForRefinements(variants[configNum],
							callingMethod);
					List<JavaVariables> refinementsVars = verifyStmt
							.generateJavaVariablesForRefinements(variants[configNum], callingMethod);
					proven = prove.proveStatementWithKey(null, refinements, refinementsVars, returnStatement, false,
							callingMethod, "", callingClass, false);
				} else {
	...
	private static boolean proveMethodStatement(AbstractStatement statement, Diagram diagram, boolean returnStatement,
			IProgressMonitor monitor) {
		if (!statement.isProven()) {
			boolean proven = false;
			...
			if (!isVariational) {
				...
				String[][] featureConfigsRelevant = VerifyFeatures.verifyConfig(uRi, varM, false, callingClass, true,
						null);
				String[][] originalFeatureConfigsRelevant = VerifyFeatures.verifyConfig(uRi, varM, false, callingClass,
						true, uri.split("/")[5]);
				...
				if (featureConfigsRelevant != null) {
					String[] variants = verifyStmt.generateVariantsStringFromFeatureConfigs(featureConfigsRelevant,
							callingFeature, varM.contains(".") ? varMParts[0] : callingClass);
					String[] variantsOriginal = verifyStmt.generateVariantsStringFromFeatureConfigs(
							originalFeatureConfigsRelevant, callingFeature, callingClass);
					if (CompareMethodBodies.readAndTestMethodBodyWithJaMoPP2(statement.getName())) {
						ProveWithKey prove = new ProveWithKey(statement, diagram, monitor,
								new FileUtil(uRi.toPlatformString(true)), featureConfigs[configNum], configNum,
								KeYInteraction.ABSTRACT_PROOF_FULL);
						...
						List<CbCFormula> refinementsOriginal = verifyStmt
								.generateCbCFormulasForRefinements(variantsOriginal[configNum], callingMethod);
						List<JavaVariables> refinementsVars = verifyStmt
								.generateJavaVariablesForRefinements(variants[configNum], varMParts[1].toLowerCase());
						proven = prove.proveStatementWithKey(refinementsOriginal, refinements, refinementsVars,
								returnStatement, false, callingMethod, varM, callingClass, false);
					} else {
						Console.println("  Statement is not in correct format.");
					}
				}
	...
	private static boolean proveCompositionStatement(AbstractStatement statement, Diagram diagram,
			IProgressMonitor monitor) {
	...
	private static boolean proveSelectionStatement(AbstractStatement statement, Diagram diagram,
			IProgressMonitor monitor) {
		boolean proven = true;
		...
		if (!(selectionStatement.isProven() && provePre && true)) {
			if (!selectionStatement.isPreProve()) {
				var uri = diagram.eResource().getURI();
				String callingFeature = FeatureUtil.getInstance().getCallingFeature(uri);
				String callingClass = FeatureUtil.getInstance().getCallingClass(uri);
				String callingMethod = FeatureUtil.getInstance().getCallingMethod(uri);
				if (isVariational) {
					String[][] featureConfigsRelevant = VerifyFeatures.verifyConfig(uri, callingMethod, true,
							callingClass, true, null);
					if (featureConfigsRelevant != null) {
						String[] variants = verifyStmt.generateVariantsStringFromFeatureConfigs(featureConfigsRelevant,
								callingFeature, callingClass);
						List<CbCFormula> refinements = verifyStmt.generateCbCFormulasForRefinements(variants[configNum],
						...
						ProveWithKey prove = new ProveWithKey(statement, diagram, monitor,
								new FileUtil(diagram.eResource().getURI().toPlatformString(true)),
								featureConfigs[configNum], configNum, KeYInteraction.ABSTRACT_PROOF_FULL);
						provePre = prove.provePreSelWithKey(refinements, guards, preCondition);
					}
				}
				if (configs == configNum)
	...
	private static boolean proveSmallReptitionStatement(AbstractStatement statement, Diagram diagram,
			IProgressMonitor monitor) {
		...
		if (repStatement.getLoopStatement().getRefinement() != null) {
			proven = (proveChildStatement(repStatement.getLoopStatement().getRefinement(), diagram, null) && proven
					&& true);
		}
		...
		if (!(repStatement.isProven() && provePre && provePost && proveVar && true)) {
			...
			if (isVariational) {
				prove = new ProveWithKey(statement, diagram, monitor,
						new FileUtil(diagram.eResource().getURI().toPlatformString(true)), featureConfigs[configNum],
			...
			List<CbCFormula> refinements = null;
			...
			if (isVariational) {
				...
				if (featureConfigsRelevant != null) {
					String[] variants = verifyStmt.generateVariantsStringFromFeatureConfigs(featureConfigsRelevant,
							callingFeature, callingClass);
					refinements = verifyStmt.generateCbCFormulasForRefinements(variants[configNum], callingMethod);
				}
			}
