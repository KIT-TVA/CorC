...
public class VerifyPostRepetitionStatementProofGraphBegin extends MyAbstractAsynchronousCustomFeature {
	 ...
	 *            The FeatureProvider
	 */
	public VerifyPostRepetitionStatementProofGraphBegin(IFeatureProvider fp) {
		super(fp);
	}

	@Override
	public String getName() {
		return "Verify the postcondition of repetition statement as proof graph (begin)";
	}
		...
		if (pes != null && pes.length == 1) {
			...
			if (bo instanceof SmallRepetitionStatement) {
				...
				for (int i = 0; i < variants.length; i++) {
					...
					proven = prove.provePostRepetitionWithKey(refinements, statement.getInvariant(),statement.getGuard(), parent.getPreCondition());
				}
