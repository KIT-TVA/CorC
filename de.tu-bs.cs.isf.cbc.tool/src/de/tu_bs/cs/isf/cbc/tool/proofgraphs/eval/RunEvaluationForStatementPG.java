...
public class RunEvaluationForStatementPG extends MyAbstractAsynchronousCustomFeature {
	...
	public void execute(ICustomContext context, IProgressMonitor monitor) {
		...
		if (pes != null && pes.length == 1) {
			...
			VerifyOriginalCallStatementProofGraphBegin pgOriginalBegin = new VerifyOriginalCallStatementProofGraphBegin(
					getFeatureProvider());
			VerifyStatementProofGraphBegin pgBegin = new VerifyStatementProofGraphBegin(getFeatureProvider());
			VerifyMethodCallStatementProofGraphBegin pgMethodBegin = new VerifyMethodCallStatementProofGraphBegin(
					getFeatureProvider());
			VerifyPreSelectionStatementProofGraphBegin pgSelection = new VerifyPreSelectionStatementProofGraphBegin(
					getFeatureProvider());
			...
			if (bo instanceof SelectionStatement) {
				...
				String preFormula = Parser
						.getConditionFromCondition(((AbstractStatement) bo).getPreCondition().getName());
				String postFormula = Parser
						.getConditionFromCondition(((AbstractStatement) bo).getPostCondition().getName());
				if (bo instanceof VerifyMethodCallStatement) {
					Console.println("==========ProofStart==========");
					manager.addJobChangeListener(listener);
					pgMethodBegin.execute(context);
				} else if (bo instanceof OriginalStatement || preFormula.contains("original")
						|| postFormula.contains("original")) {
					Console.println("==========ProofStart==========");
					manager.addJobChangeListener(listener);
	...
	private class MyJobChangeListener implements IJobChangeListener {
		...
		public void done(IJobChangeEvent event) {
			...
			VerifyOriginalCallStatementProofGraphBegin pgOriginalBegin = new VerifyOriginalCallStatementProofGraphBegin(
					getFeatureProvider());
			VerifyOriginalCallStatementProofGraphComplete pgOriginalComplete = new VerifyOriginalCallStatementProofGraphComplete(
					getFeatureProvider());
			VerifyMethodCallStatementProofGraphBegin pgMethodBegin = new VerifyMethodCallStatementProofGraphBegin(
					getFeatureProvider());
			VerifyMethodCallStatementProofGraphComplete pgMethodComplete = new VerifyMethodCallStatementProofGraphComplete(
					getFeatureProvider());
			VerifyPreSelectionStatementProofGraphBegin pgSelectionBegin = new VerifyPreSelectionStatementProofGraphBegin(
					getFeatureProvider());

			if (event.getJob().getName().equals(pgBegin.getName())
				...
				if (RunEvaluationForStatementPP.current < rounds) {
					...
					if (proofStart && RunEvaluationForStatementPP.current != 1) {
						...
						RunEvaluationForStatementPP.NODES_COMPLETE
								.add(RunEvaluationForStatementPP.currentProofNodes + "");
						RunEvaluationForStatementPP.currentProofTimes = 0;
						RunEvaluationForStatementPP.currentProofNodes = 0;
					} else {
						RunEvaluationForStatementPP.PROOF_RUNTIME_START
								.add(RunEvaluationForStatementPP.currentProofTimes + "");
