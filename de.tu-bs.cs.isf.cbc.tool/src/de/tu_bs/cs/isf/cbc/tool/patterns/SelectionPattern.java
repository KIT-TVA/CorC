...
public class SelectionPattern extends IdPattern implements IPattern {
	...
	private static final String ID_VER4_LINE = "ver4Line"; //in statements
	...
	public Object[] create(ICreateContext context) {
		...
		Condition preSel = CbcmodelFactory.eINSTANCE.createCondition();
		preSel.setName("");
		selectionStatement.setPreCondition(preSel);
		Condition postSel = CbcmodelFactory.eINSTANCE.createCondition();
		postSel.setName("");
		selectionStatement.setPostCondition(postSel);
...
//		link(textShapeStatement, addedStatement.getCommands().get(0));
	...
	protected boolean layout(IdLayoutContext context, String id) {
		...
		//Header:
			...
			int index = getIndex(context.getGraphicsAlgorithm());
			Graphiti.getGaService().setLocationAndSize(ga, width * index, positionLine4, width, sizeBlock);
			changesDone = true;
		} else if (id.equals(ID_GUARD_HEADER)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionLine1 - sizeHeader, mainRectangle.getWidth(), sizeHeader);
			changesDone = true;
		} else if (id.equals(ID_PRE_HEADER)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionLine2 - sizeHeader, mainRectangle.getWidth(), sizeHeader);
			changesDone = true;
		} else if (id.equals(ID_STATEMENT_HEADER)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionLine3 - sizeHeader, mainRectangle.getWidth(), sizeHeader);
			changesDone = true;
		} else if (id.equals(ID_POST_HEADER)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionLine4 - sizeHeader, mainRectangle.getWidth(), sizeHeader);
			changesDone = true;
		//lines:
			...
			if (index > 0) {
				...
				polyline.getPoints().clear();
				List<Point> pointList = Graphiti.getGaService().createPointList(
						new int[] { width * index, positionLine4,  width * index, mainRectangle.getHeight() });
				polyline.getPoints().addAll(pointList);
			} else {
				Polyline polyline = (Polyline) ga;
				polyline.getPoints().clear();
	...
	protected boolean update(IdUpdateContext context, String id) {
		if (id.equals(ID_MAIN_RECTANGLE)) {
			...
			while (containerShape.getChildren().size() - 14 != statement.getCommands().size() * 8) {
				System.out.println(containerShape.getChildren().size() - 14);
				System.out.println(statement.getCommands().size() * 8);
...
//				int newIndex = childStatements.size() - 1;
				...
				Shape ver4Shape = peCreateService.createShape((ContainerShape) context.getPictogramElement(), false);
				Polyline ver4line = Graphiti.getGaService().createPolyline(ver4Shape);
				setId(ver4line, ID_VER4_LINE);
				setIndex(ver4line, newIndex);
				...
				Shape shapeTextCond = Graphiti.getPeCreateService().createShape((ContainerShape) context.getPictogramElement(), true);
