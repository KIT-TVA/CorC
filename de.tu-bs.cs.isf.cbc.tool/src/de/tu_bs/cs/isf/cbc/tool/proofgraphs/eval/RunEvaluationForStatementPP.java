...
public class RunEvaluationForStatementPP extends MyAbstractAsynchronousCustomFeature {
	...
	public void execute(ICustomContext context, IProgressMonitor monitor) {
		...
		if (pes != null && pes.length == 1) {
			...
			VerifyOriginalCallStatementPartialProofBegin ppOriginalBegin = new VerifyOriginalCallStatementPartialProofBegin(
					getFeatureProvider());
			VerifyStatementPartialProofBegin ppBegin = new VerifyStatementPartialProofBegin(getFeatureProvider());
			VerifyMethodCallStatementPartialProofBegin ppMethodCallStatement = new VerifyMethodCallStatementPartialProofBegin(
					getFeatureProvider());
	...
	private class MyJobChangeListener implements IJobChangeListener {
		...
		public void done(IJobChangeEvent event) {
			VerifyStatementPartialProofBegin ppStatementBegin = new VerifyStatementPartialProofBegin(
					getFeatureProvider());
			VerifyStatementPartialProofComplete ppStatementComplete = new VerifyStatementPartialProofComplete(
					getFeatureProvider());
			VerifyOriginalCallStatementPartialProofBegin ppOriginalBegin = new VerifyOriginalCallStatementPartialProofBegin(
					getFeatureProvider());
			VerifyOriginalCallStatementPartialProofComplete ppOriginalComplete = new VerifyOriginalCallStatementPartialProofComplete(
					getFeatureProvider());
			VerifyMethodCallStatementPartialProofBegin ppMethodCallStatementBegin = new VerifyMethodCallStatementPartialProofBegin(
					getFeatureProvider());
			VerifyMethodCallStatementPartialProofComplete ppMethodCallStatementEnd = new VerifyMethodCallStatementPartialProofComplete(
					getFeatureProvider());
			if (event.getJob().getName().equals(ppStatementBegin.getName())
				...
				if (current < rounds) {
					...
					Console.println(
							Arrays.toString(WHOLE_RUNTIME_COMPLETE.toArray()).replace("[", "").replace("]", ""));
					Console.println(Arrays.toString(PROOF_RUNTIME_START.toArray()).replace("[", "").replace("]", ""));
					Console.println(
							Arrays.toString(PROOF_RUNTIME_COMPLETE.toArray()).replace("[", "").replace("]", ""));
