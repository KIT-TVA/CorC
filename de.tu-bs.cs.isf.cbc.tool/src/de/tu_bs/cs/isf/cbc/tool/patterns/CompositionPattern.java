...
public class CompositionPattern extends IdPattern implements IPattern {
	...
	public PictogramElement doAdd(IAddContext context) {
		...
		MultiText conditionText = gaService.createMultiText(textShapeCondition,
				"{" + addedStatement.getIntermediateCondition().getName() + "}");
		...
		MultiText pre1NameText = gaService.createMultiText(pre1Shape,
				"{" + addedStatement.getFirstStatement().getPreCondition().getName() + "}");
		...
		MultiText post2NameText = gaService.createMultiText(post2Shape,
				"{" + addedStatement.getSecondStatement().getPostCondition().getName() + "}");
	...
	protected boolean layout(IdLayoutContext context, String id) {
		...
		int HeightWithoutName = mainRectangle.getHeight() - sizeName; // size from the statement without the Name block,
																		// -20 for some space
		int sizeHeader = 20;
		...
		if (id.equals(ID_NAME_TEXT)) {
			...
			Graphiti.getGaService().setLocationAndSize(ga, thirdWidth, positionLine2 + sizeHeader + blockSize / 3,
					thirdWidth, 2 * blockSize / 3);
			changesDone = true;
		} else if (id.equals(ID_CONDITION_MOD)) {
			Graphiti.getGaService().setLocationAndSize(ga, thirdWidth, positionLine2 + sizeHeader, thirdWidth,
					blockSize / 3);
			changesDone = true;
		} else if (id.equals(ID_ST2_TEXT)) {
			Graphiti.getGaService().setLocationAndSize(ga, thirdWidth * 2, positionLine2 + sizeHeader, thirdWidth,
					blockSize);
			changesDone = true;
		} else if (id.equals(ID_PRE1_TEXT)) {
			Graphiti.getGaService().setLocationAndSize(ga, 0, positionLine1 + sizeHeader + blockSize / 3, halfWidth,
					2 * blockSize / 3);
			changesDone = true;
			...
			Graphiti.getGaService().setLocationAndSize(ga, halfWidth, positionLine1 + sizeHeader + blockSize / 3,
					halfWidth, 2 * blockSize / 3);
			changesDone = true;
		} else if (id.equals(ID_POST2_MOD)) {
			Graphiti.getGaService().setLocationAndSize(ga, halfWidth, positionLine1 + sizeHeader, halfWidth,
					blockSize / 3);
			changesDone = true;
			...
			polyline.getPoints().clear();
			List<Point> pointList = Graphiti.getGaService()
					.createPointList(new int[]{thirdWidth, positionLine2 + sizeHeader + blockSize / 3, thirdWidth * 2,
							positionLine2 + sizeHeader + blockSize / 3});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
			...
			List<Point> pointList = Graphiti.getGaService().createPointList(new int[]{0,
					positionLine1 + sizeHeader + blockSize / 3, halfWidth, positionLine1 + sizeHeader + blockSize / 3});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
			...
			polyline.getPoints().clear();
			List<Point> pointList = Graphiti.getGaService()
					.createPointList(new int[]{halfWidth, positionLine1 + sizeHeader + blockSize / 3,
							mainRectangle.getWidth(), positionLine1 + sizeHeader + blockSize / 3});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
			...
			polyline.getPoints().clear();
			List<Point> pointList = Graphiti.getGaService()
					.createPointList(new int[]{halfWidth, positionLine1, halfWidth, positionLine2});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
			...
			polyline.getPoints().clear();
			List<Point> pointList = Graphiti.getGaService()
					.createPointList(new int[]{thirdWidth, positionLine2, thirdWidth, mainRectangle.getHeight()});
			polyline.getPoints().addAll(pointList);
			changesDone = true;
