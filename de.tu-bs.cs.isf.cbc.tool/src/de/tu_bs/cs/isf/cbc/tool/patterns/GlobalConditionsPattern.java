...
public class GlobalConditionsPattern extends IdPattern implements IPattern {
	...
	protected boolean update(IdUpdateContext context, String id) {
		if (id.equals(ID_MAIN_RECTANGLE)) {
			...
			while (((ContainerShape) context.getPictogramElement()).getChildren().size() - 1 < conditions.size()) {
				int newIndex = ((ContainerShape) context.getPictogramElement()).getChildren().size() - 1;
