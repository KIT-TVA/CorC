...
/**
 * Translates an AST into the syntax of the Z3-Solver. Z3 is used to get values
 * for variables given multiple preconditions. Currently only supports the
 * primitive types [boolean, byte, short, int, long].
 * 
 * @author Fynn Demmler
 */
public class PreConditionSolver {
	 ...
	 * 
	 * @param preConditions
	 *            All preconditions (precondition from the hoare tripel + global
	 *            invariants) in JavaDL syntax.
	...
	private Expr makeCondition(Node tree) {
		...
		FileHandler.instance
				.log("PreConditionSolverWarning: Type of identifier '" + tree.getRep() + "' is not supported.");
		return context.mkTrue();
	}
	 ...
	 * @return The index.
	 */
	private int evalIndexTree(final Node indexTree, final List<Expr> vars, final Model model)
			throws IdentifierNotFoundException, PreConditionSolverException {
		int curVal = 0;
		...
		if (indexTree.getType() == TokenType.NUMBER) {
			...
			switch (curNode.getOperator()) {
				case "+" :
					curVal = evalIndexTree(curNode.getLeft(), vars, model)
							+ evalIndexTree(curNode.getRight(), vars, model);
				case "-" :
					curVal = evalIndexTree(curNode.getLeft(), vars, model)
							- evalIndexTree(curNode.getRight(), vars, model);
				case "*" :
		...
		return curVal + evalIndexTree(indexTree.getLeft(), vars, model)
				+ evalIndexTree(indexTree.getRight(), vars, model);
	}

	private boolean constructData(Expr v, List<InputData> output, final ArrayList<Expr> vars, final Model model)
			throws PreConditionSolverException, IdentifierNotFoundException, UnexpectedTokenException,
			SettingsException {
		...
		if (jVars != null) {
			if (v.toString().equals("null")) {
				...
				String refVar = name.substring(name.indexOf("[") + 1, name.indexOf("]")); // TODO: Parse this term to
																							// get the actual var
	...
	public List<InputData> solve(final String preCon)
			throws PreConditionSolverException, UnexpectedTokenException, SettingsException {
		...
		try {
			...
			throw e;
			/*
			 * if (showWarnings) Console.
			 * println("SolverWarning: Defaulting to precondition 'true' because of one or more previous errors."
