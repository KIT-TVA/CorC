...
public class OriginalStatementPattern extends IdPattern implements IPattern {
	...
	private static final String ID_NAME_HEADER2 = "statementHeader2";
	// Separating lines:
	 ...
	 */
	public OriginalStatementPattern() {
		super();
	}

	@Override
	public String getCreateName() {
		return "Original-Call Statement";
	}

	@Override
	public String getCreateDescription() {
		return "Create an Original Statement.";
	}

	@Override
	public boolean isMainBusinessObjectApplicable(Object mainBusinessObject) {
		return mainBusinessObject instanceof OriginalStatement;
	}
	...
	public Object[] create(ICreateContext context) {
		OriginalStatement statement = CbcmodelFactory.eINSTANCE.createOriginalStatement();//
		statement.setName("original();");
	...
	public PictogramElement doAdd(IAddContext context) {
		...
		OriginalStatement addedStatement = (OriginalStatement) context.getNewObject();
		...
		Text statementNameHeader = gaService.createText(textHeader, "Original-Call");
		...
		// Header:---------------
		Shape textHeader2 = peCreateService.createShape(outerContainerShape, false);
		Text statementNameHeader2 = gaService.createText(textHeader2, "Statement");
		setId(statementNameHeader2, ID_NAME_HEADER2);
		statementNameHeader2.setHorizontalAlignment(Orientation.ALIGNMENT_CENTER);
		statementNameHeader2.setVerticalAlignment(Orientation.ALIGNMENT_CENTER);
		statementNameHeader2.setFont(headerFont);
	...
	protected boolean layout(IdLayoutContext context, String id) {
		...
		if (id.equals(ID_NAME_TEXT)) {
			...
			Graphiti.getGaService().setLocationAndSize(ga, third, 5, third, 20); // mainrectangle anpassen
			changesDone = true;
		} else if (id.equals(ID_NAME_HEADER2)) {
	...
	protected IReason updateNeeded(IdUpdateContext context, String id) {
		if (context.getGraphicsAlgorithm() instanceof MultiText
				&& context.getDomainObject() instanceof OriginalStatement) {
			MultiText nameText = (MultiText) context.getGraphicsAlgorithm();
			OriginalStatement domainObject = (OriginalStatement) context.getDomainObject();
	...
	public boolean canDirectEdit(IDirectEditingContext context) {
		...
		GraphicsAlgorithm ga = context.getGraphicsAlgorithm();
		if (domainObject instanceof OriginalStatement && ga instanceof MultiText) {
			return true;
		}
		return false;
	}
	...
	public String getInitialValue(IDirectEditingContext context) {
		OriginalStatement statement = (OriginalStatement) getBusinessObjectForPictogramElement(
	...
	public String checkValueValid(String value, IDirectEditingContext context) {
		...
		if (Pattern.matches(".*original[(].*[)].*", value) == false) {
			return "Statement must contain an original-call";
		} else if (Pattern.matches(".*original[(][^.[(]]*[)].*", value) == false) {
			return "No nested method-calls in an original-call allowed.";
		}
		return null;
	}
