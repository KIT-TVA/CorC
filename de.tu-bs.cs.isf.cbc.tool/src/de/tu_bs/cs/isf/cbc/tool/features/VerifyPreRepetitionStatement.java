			...
			if (bo instanceof SmallRepetitionStatement) {
				...
				if (isVariational) {
					...
					for (int i = 0; i < featureConfigs.length; i++) {
						...
						proven = prove.proveCImpliesCWithKey(parent.getPreCondition(), statement.getInvariant(), i);
					}
				} else {
					Console.println("--------------- Triggered verification ---------------");
					proven = prove.proveCImpliesCWithKey(parent.getPreCondition(), statement.getInvariant(), 0);
				}		
