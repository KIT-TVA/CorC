...
public class ConditionPattern extends IdPattern implements IPattern {
	...
	if (id.equals(ID_PRE_MOD) || id.equals(ID_POST_MOD) || id.equals(ID_OTHER_MOD)) {
	    ...
	    return System.getProperty("line.separator") + ("modifiable("
		    + (modString.equals("") ? "" : modString.substring(0, modString.length() - 2)) + ");")
	...
	if (id.equals(ID_PRE_MOD) || id.equals(ID_POST_MOD) || id.equals(ID_OTHER_MOD)) {
	    ...
	    for (String s : split) {
		if (!s.equals(""))
		    condition.getModifiables().add(s.trim());
	    }
	...
	if (condition.eContainer() instanceof ModelClass) {
	    ...
	    if (peURI.lastSegment().contains(FeatureUtil.getInstance().getCallingClass(peURI))) { // change from
												  // CbCClass
		try {
	...
	if (condition.eContainer() != null && (condition.eContainer() instanceof GlobalConditions)
		|| condition.eContainer() instanceof ModelClass) {
	...
	if (statement instanceof SmallRepetitionStatement) {
		...
		final AbstractStatement returnValue = findStatementForCondition(condition,
			repetition.getLoopStatement());
		if (returnValue != null) {
		    return returnValue;
		}
	    }
