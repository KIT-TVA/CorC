...
/**
 * This class holds a parser, which parses the presence condition of variable
 * predicates. It parses them into a set of partial feature configurations which
 * can then be used to calculate all required configurations for a full proof of
 * such a variable predicate.
 * 
 * @author Markus
 */
public class PresenceConditionParser {
	...
	public List<Set<SelectionInfo>> evaluateAST(PresenceConditionNode node, boolean not) {
		...
		if (node instanceof PresenceConditionFeatureNode featureNode) {
			...
			current.forEach(list -> list
					.add(new SelectionInfo(featureNode.getName(), !not ? Selection.SELECTED : Selection.UNSELECTED)));
		}
	...
	private PresenceConditionNode parseFeature() {
		...
		if (this.featureList != null && !this.featureList.isEmpty()) {
			if (!this.featureList.contains(featureName)) {
				String errorInfo = presenceCondition.substring(0, cursor - 1) + "  <-ERROR-  "
						+ presenceCondition.substring(cursor);
