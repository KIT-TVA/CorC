...
public class RHelper {
	...
	private boolean generate(String header, String rootLocation, String fileName, List<StatisticsEntry> entries,
		...
		try {
			...
			try (Reader reader = new BufferedReader(
					new InputStreamReader(rProcess.getErrorStream(), Charset.forName(StandardCharsets.UTF_8.name())))) {
				int c = 0;
		...
		for (String diagramName : diagramNames) {
			...
			for (final String config : configs) {
				for (final StatisticsEntry entry : this.configEntries.keySet()) {
					if (entry.getMapping().getCorcDiagramName().equals(diagramName)
							&& config.equals(this.configEntries.get(entry))) {
	...
	private String builtPlot(DiagramAxes axes, boolean stepPlot) {
		...
		if (stepPlot) {
			plotCommand = "barplot(steps, ylab = \"Total number of proof steps\", col=" + colors
					+ ", names.arg=diagram, las=2, ylim=c(0, ceiling(max(steps, na.rm=TRUE)) + ceiling(max(steps, na.rm=TRUE)*0.3)))\r\n";
		} else {
			if (axes.isAverageData()) {
				plotCommand = "barplot(time, ylab = \"Average AutoMode time [ms]\", col=" + colors
						+ ", names.arg=diagram, las=2, ylim=c(0, ceiling(max(time, na.rm=TRUE)) + ceiling(max(time, na.rm=TRUE)*0.3)))\r\n";
			} else {
				plotCommand = "barplot(time, ylab = \"AutoMode time [ms]\", col=" + colors
						+ ", names.arg=diagram, las=2, ylim=c(0, ceiling(max(time, na.rm=TRUE)) + ceiling(max(time, na.rm=TRUE)*0.3)))\r\n";
			}
		}
