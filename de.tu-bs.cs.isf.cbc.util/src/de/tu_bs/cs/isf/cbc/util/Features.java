...
public class Features {
	...
	public Features(final URI uri) throws FeatureCallerException {
		...
		featureConfigs = VerifyFeatures.verifyConfig(uri, uri.segment(uri.segmentCount() - 1), true, callingClass,
				false, null);
	...
	public CbCFormula loadFormulaFromFeature(final IFeatureProvider fp, final String featureName,
			final String className, final String methodName) {
		if (featureName.isBlank() || methodName.isBlank()) {
			return null;
		}
		final ResourceSet rSet = new ResourceSetImpl();
		final IFolder folder = FileUtil.getProject(uri).getFolder(FeatureUtil.FEATURE_FOLDER).getFolder(featureName)
				.getFolder(className);
		if (folder == null) {
			return null;
		}
