...
/**
 * Provides all functions that handle the keyword original in conditions and
 * code. This class also handles abstract method calls. Note: This class doesn't
 * use the already existing code generation for SPLs, because that generates all
 * code at once, which could lead to performance issues. Lazy generation of code
 * and conditions is used instead.
 * 
 * @author Fynn Demmler
 */
public class TestUtilSPL {
     ...
     * @param signature
     * @param features
     * @return The new condition in which all occurrences of the original keywords
     *         were replaced.
     * @throws ReferenceException
     */
    public String handleOriginalCondition(final IFeatureProvider fp, String condition, boolean isPreCon,
	    final Features features) throws ReferenceException {
	...
	CbCFormula originalFormula = features.loadFormulaFromFeature(fp, refFeature, features.getCallingClass(),
		features.getCallingMethod());
	...
	return condition = condition.substring(0, condition.indexOf("original")) + "(" + originalCondition + ")"
		+ condition.substring(condition.indexOf("original") + "original".length(), condition.length());
    }
	...
	return signature.substring(0, signature.indexOf(features.getCallingMethod())) + "original_"
    ...
    public void handleOriginalCode(final IFeatureProvider fp, final URI projectPath, String code,
	    final Features features, final List<MethodHandler> newMethods, String signature, final JavaVariables vars,
	    final JavaVariable returnVar)
	    throws ReferenceException, TestAndAssertionGeneratorException, SettingsException {
	...
	CbCFormula originalFormula = features.loadFormulaFromFeature(fp, refFeature, features.getCallingClass(),
		features.getCallingMethod());
	if (originalFormula == null) {
	    handleOriginalCode(fp, projectPath, code, features, newMethods, signature, vars, returnVar);
	    return;
	}
	String refCode = CodeGenerator.instance.genDiagramCode(originalFormula, null);
	...
	refCode = insertReturnVar(refCode, returnVar);
	...
	handleOriginalCode(fp, projectPath, refCode, features, newMethods, signature, vars, returnVar);
    }

    private String insertReturnVar(String code, JavaVariable returnVar) throws SettingsException {
	if (returnVar == null) {
	    return code;
	}
	String type = returnVar.getName().split("\\s")[0];
	String name = returnVar.getName().split("\\s")[1];
	if (code.contains("var " + returnVar.getName()) || code.contains(type + " " + name)) {
	    return code;
	}
	InputData uData;
	uData = new InputData(name, type);
	uData.setDefaultValues();
	var val = uData.getRandomValue();
	return type + " " + name + " = " + val + ";\n" + code;
    }

    private String getRefFeature(final Features features, final String callingFeature, String[] curConfig)
	    throws ReferenceException {
    ...
    /**
     * Handles abstract function calls inside a SPL. A function is abstract if it's
     * definition depends on the configuration it is called in. For example, if the
     * function 'sort' in the feature 'sorted' is called, the configuration must
     * contain feature 'increasing' or feature 'decreasing'. The definition of the
     * function 'sort' will then be extracted from the one existing in the current
     * configuration.
     * 
     * @param fp
     * @param projectPath
     ...
     */
    public List<MethodHandler> handleAbstractMethodCalls(final IFeatureProvider fp, final URI projectPath,
	    final String code, final Features features, final List<MethodHandler> newMethods)
	...
	for (var mName : methodNames) {
		...
		Diagram diag = FileHandler.instance.loadDiagramFromClass(projectPath, "features/" + feature, mName);
		if (diag == null) {
		    continue;
		}
		String functionCode = CodeGenerator.instance.genCode(diag, true);
    ...
    public void addNewMethods(final List<ClassHandler> classCodes, final String className,
	    final ArrayList<MethodHandler> originalMethods, final ArrayList<MethodHandler> abstractMethods) {
