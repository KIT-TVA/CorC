...
/**
 * This class can be used to translate conditions represented as an AST into
 * java.
 * 
 * @author Fynn
 */
public class JavaCondition {
	...
	while (unchangedCode.contains("if (") || unchangedCode.contains("Assert.assertTrue")) {
	    ...
	    int assertIndex = unchangedCode.indexOf("Assert") == -1 ? Integer.MAX_VALUE
		    : unchangedCode.indexOf("Assert");
	    int end = 0;
		...
		code = code.substring(0, start + offset) + "context.setAttribute(\"" + testNumber + "branch"
			+ branchCounter + "\", \"" + branchCondition + "\");" + "\n"
		...
		List<String> varNames = Variable.getVarNames(assertion,
			inputs.get(0).getInputDataTupel().getGlobalVarNames(),
			inputs.get(0).getInputDataTupel().getParameterNames(), instanceName);
		start = assertIndex;
		for (var v : varNames) {
		    code = code.substring(0, start + offset) + "context.setAttribute(\"" + testNumber + v + "\", " + v
			    + ");" + "\n" + code.substring(start + offset, code.length());
		}
    ...
    // TODO: Currently, this expects forall's to have the following structure q > 0
    // && q < A.length.
	...
	var firstCon = "";
	Node current = quantor.getConditions().getLeft();
	while (current.getLeft() != null && current.getLeft().getLeft() != null) {
	    current = current.getLeft();
	    isLayered = true;
	}
	firstCon = current.getRep();
	final String firstCondition = removeIdentifier(firstCon, quantor.getIterator());
	final String initialValue = getIterValue(firstCondition);
	if (isLayered) {
	    current = quantor.getConditions().getLeft();
	    while (current.getRight() != null && current.getRight().getRight() != null) {
		current = current.getRight();
	    }
	} else {
	    current = quantor.getConditions().getRight();
	    while (current.getLeft() != null && current.getLeft().getLeft() != null) {
		current = current.getLeft();
	    }
	}
	var guard = current;
	final String secondCondition = removeIdentifier(guard.getRep(), quantor.getIterator());
	final String lastValue = getIterValue(secondCondition);
	final String name = getPredicateName();

	condition += "for (" + quantor.getIteratorType() + " " + quantor.getIterator() + " = " + initialValue + "; "
		+ quantor.getIterator() + " < " + lastValue + "; " + quantor.getIterator() + "++" + ") " + "{\n";
	final String innerCondition = translateTreeToJava(quantor.getConditions().getRight());
	...
	while (start != -1) {
	    ...
	    condition = condition.substring(0, start) + "if " + condition.substring(start + target.length(), end)
	...
	while (start != -1) {
		...
		condition = condition.substring(0, start) + "if " + condition.substring(start + target.length(), end)
			+ " loopCounter++;\n" + condition.substring(end + 2, condition.length());
		condition = condition.substring(0, blockEndIndex) + "if (loopCounter == " + loopBound + ") " + varName
