...
public class CodeGenerator {
	...
	for (int i = 0; i < vars.getVariables().size(); i++) {
		...
		if (!signatureString.substring(0, signatureString.indexOf('('))
		    ...
		    // throw new CodeGeneratorException("Method return type and variable type does
		    // not match.");
		}
		if (counter > 1) {
	...
	if (FileHandler.instance.isSPL(uri)) {
	    var genCode = new GenerateCodeForVariationalVerification(null);
	    genCode.setDiagram(diag);
	    String[][] featureConfigs = VerifyFeatures.verifyConfig(uri, uri.segment(uri.segmentCount() - 1), true,
		    FeatureUtil.getInstance().getCallingClass(uri), false, null);
	    genCode.generate(FileUtil.getProjectFromFileInProject(uri).getLocation(),
		    FeatureUtil.getInstance().getCallingFeature(uri), FeatureUtil.getInstance().getCallingClass(uri),
		    FeatureUtil.getInstance().getCallingMethod(uri), featureConfigs[0]);
	    code = genCode.getFinalMethodCode();
	} else {
	    code = ConstructCodeBlock.constructCodeBlockForExport(formula, globalConditions, renaming, localVariables,
