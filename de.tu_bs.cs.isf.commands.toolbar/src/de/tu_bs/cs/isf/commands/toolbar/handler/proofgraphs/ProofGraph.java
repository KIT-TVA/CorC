...
public class ProofGraph {
	...
	public void createNode(IFeature feature, String method) {
		this.adjacencyList.computeIfAbsent(new ProofNode(feature.getName(), method, getIdForFeature(feature.getName())),
				a -> new HashSet<ProofNode>());
	}
	...
	public void cleanGraph() {
		this.adjacencyList.forEach((caller, called) -> {
			...
			// ########
			/*
			 * List<ProofNode> toRemove = new ArrayList<ProofNode>(); called.forEach(val ->
			 * { List<String> intersection =
			 * caller.getImplementedMethods().stream().filter(method ->
			 * val.getImplementedMethods().contains(method)).toList(); // No common methods
			 * if (intersection.isEmpty()) { toRemove.add(val); } });
			 * this.adjacencyList.get(caller).removeAll(toRemove);
			 */

			List<ProofNode> toRemove = new ArrayList<ProofNode>();
			called.forEach(val -> {
				// ########## STEP 1 - Remove connections to methods that don't
				// implement########
				List<String> intersection = this.getImplementedMethods(caller.getFeature()).stream()
						.filter(method -> this.getImplementedMethods(val.getFeature()).contains(method)).toList();
				...
				for (SelectableFeature feature : configuration.getFeatures()) {
					if (feature.getSelection() != Selection.UNDEFINED) {
						solver.assignmentPush(vars.getVariable(feature.getFeature().getName(),
			...
			called.forEach(val -> {
				...
				for (SelectableFeature current : configuration.getFeatures()) {
					if (current.getSelection() != Selection.UNDEFINED) {
						solver.assignmentPush(vars.getVariable(current.getFeature().getName(),
				...
				if (solver.hasSolution() == SatResult.FALSE) {
					...
					for (int i = index - 1; i >= 0; i--) {
						String featToRemove = featureModel.getFeature(featureModel.getFeatureOrderList().get(i))
								.getName();
						this.featureMap.get(featToRemove).forEach(method -> toRemove
								.add(new ProofNode(featToRemove, method, getIdForFeature(featToRemove))));
					}
				}
			});
	...
	public boolean equals(Object obj) {
		if (obj instanceof ProofGraph graph) {
			List<ProofNode> sortedA = new ArrayList<>(
					this.adjacencyList.keySet().stream().map(ProofNode::getWithGeneralMethod).toList());
			List<ProofNode> sortedB = new ArrayList<>(
					graph.adjacencyList.keySet().stream().map(ProofNode::getWithGeneralMethod).toList());
			...
			if (!sortedA.equals(sortedB)) {
				// Console.println(String.format("Different keySets (%s, %s)", sortedA,
				// sortedB));
				return false;
			}
			...
			for (ProofNode key : this.adjacencyList.keySet()) {
				if (graph.adjacencyList.get(key) != null) {
					...
					List<ProofNode> sortedAEdges = new ArrayList<>(
							this.adjacencyList.get(key).stream().map(ProofNode::getWithGeneralMethod).toList());
					List<ProofNode> sortedBEdges = new ArrayList<>(
							graph.adjacencyList.get(key).stream().map(ProofNode::getWithGeneralMethod).toList());
