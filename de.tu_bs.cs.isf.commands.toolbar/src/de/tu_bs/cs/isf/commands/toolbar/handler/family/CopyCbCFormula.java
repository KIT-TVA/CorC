...
public class CopyCbCFormula {
	...
	/*
	 * private static void copyRepetitionStatement(AbstractStatement
	 * previousStatement, RepetitionStatement repetitionStatement) { Console.
	 * println("ERROR: CopyFormula: Big RepettionStatement Copy Not Implemented"); }
	 */

	private static void copySelectionStatement(AbstractStatement previousStatement,
			SelectionStatement selectionStatementToCopy) {
		...
		for (int i = 0; i < selectionStatementToCopy.getCommands().size(); i++) {
			...
			copyAllRefinements(newSelection.getCommands().get(i),
					selectionStatementToCopy.getCommands().get(i).getRefinement());
		}

	}

	private static void copyCompositionStatement(AbstractStatement previousStatement,
			CompositionStatement compositionStatementToCopy) {
		...
		copyAllRefinements(newComposition.getFirstStatement(),
				compositionStatementToCopy.getFirstStatement().getRefinement());
		copyAllRefinements(newComposition.getSecondStatement(),
				compositionStatementToCopy.getSecondStatement().getRefinement());
	...
	private static void copySmallRepetitionStatement(AbstractStatement previousStatement,
			SmallRepetitionStatement repetitionStatementToCopy) {
		...
		copyAllRefinements(newRepetitionStatement.getLoopStatement(),
				repetitionStatementToCopy.getLoopStatement().getRefinement());
	}
	...
	public static JavaVariable copyJavaVariable(JavaVariable variableToCopy) {
		...
		// newJavaVariable.setDisplayedName(new
		// String(variableToCopy.getDisplayedName()));
