...
public class IFbCMethod extends IFbcAbstractStatement {
	...
	public IFbCMethod(final String name, final IFbCMethodEntity entity) {
		this.name = name;
		this.entity = entity;
	}
	...
	public TypeableResult isTypeable(final Lattice lattice, final String projectName,
		...
		final String definingClassName = changedTypes.get(entity.getName()) != null
				? changedTypes.get(entity.getName())
				: entity.getDefiningClassName();
		...
		for (int i = 0; i < parsedParameters.size(); i++) {
			...
			System.out.println("Checking SL for parameters. parsed param SL: " + parsedParamSLNode.getName()
					+ ", defined param SL: " + definedParamSLNode.getName());
		...
		if (!method.getStaticMethod()) {
			// check if defined and runtime receiver are matching, i.e. runtime is >=
			// defined
			...
			if (!definedReceiverSL.equals(runtimeReceiverSL)
					&& LeastUpperBound.secondHigherThanFirst(runtimeReceiverSLNode, definedReceiverSLNode)) {
				return new TypeableResult(false,
						"Defined receiver SL (" + definedReceiverSL + ") for method " + entity.getName()
								+ " is higher than runtime SL for receiver (" + runtimeReceiverSL + ").",
						elevatedEntities, changedTypes, usedCapsules);
			}
			...
			if (!method.getVoidMethod()) {
				...
				if (!returnMDF.equals(MDF.IMMUTABLE) && !returnMDF.equals(MDF.CAPSULE)
						&& !returnSL.equals(runtimeReceiverSL)
		...
		if (!method.getStaticMethod()) {
			...
			if (runtimeReceiverSLNode == null) {
				throw new IFbCException("SL " + runtimeReceiverSL + " for runtime receiver SL could not be found: "
						+ runtimeReceiverSL);
			}

			if (!runtimeReceiverSL.equals(this.statementSL)
					&& LeastUpperBound.secondHigherThanFirst(statementSLNode, runtimeReceiverSLNode)) {
				if (parsedParameters == null || parsedParameters.isEmpty()) {
					return new TypeableResult(false, "Runtime receiver SL (" + runtimeReceiverSL
							+ ") is higher than expression SL (" + this.statementSL
							+ "). Need to use parameters with a higher SL. No automatic promotion possible without parameters.",
							elevatedEntities, changedTypes, usedCapsules);
				}

				for (ParameterDefinition parsedParameter : parsedParameters) {
					// Check if SL of receiver is reachable from the parameter SL
					final TypeableResult checkParameterDefintion = checkParameterDefintion(lattice, projectName,
							elevatedEntities, changedTypes, usedCapsules, javaClassesForProject, parsedParameter);
					...
					if (!isSLPromotable(parameterSLNode, runtimeReceiverSLNode)) {
						return new TypeableResult(false,
								"Runtime receiver security level (" + runtimeReceiverSLNode.getName()
										+ ") is not reachable from parsed parameter security level ("
										+ parameterSLNode.getName() + "). Cannot locate parameter locally.",
								elevatedEntities, changedTypes, usedCapsules);
					}
					...
					if (!parameterMDF.equals(MDF.IMMUTABLE) && !parameterMDF.equals(MDF.CAPSULE)) {
						return new TypeableResult(false,
								"Runtime receiver SL (" + runtimeReceiverSL + ") is higher than expression SL ("
										+ this.statementSL + "). " + "Need to use parameters with a higher SL. "
										+ "\n\tNo automatic promotion of parameters possible since MDF of parameter '"
										+ parsedParameter.getName() + "' is not IMM|CAPS.",
								elevatedEntities, changedTypes, usedCapsules);
					}
				}

				System.out.println("Local promotion of all parameters of method call " + method.getName()
						+ " was possible to achieve receiver SL " + runtimeReceiverSL + ".");
			}
		}
		...
		return new TypeableResult(true, "Method  " + entity.getStatement() + " is typeable.", elevatedEntities,
				changedTypes, usedCapsules);
	}
