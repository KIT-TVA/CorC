...
public class JavaClass {
	...
	public JavaClass(String className, String sourceFile, Long parseTime, List<IFieldOrMethod> v,
			ClassSubtypes subtypes, ConstructorDefinition constructor) {
		super();
	...
	/**
	 * Returns the methods for which the method name and the size of
	 * parameters/arguments are the same in this class
	 * 
	 * @param methodCallExpr
	 * @param diagramVariables
	 * @return
	 */
	public Method getMethodForMethodCallExpr(final MethodCallExpr methodCallExpr,
			Map<String, IFbCReferenceEntity> diagramVariables) {
		final List<Method> methods = getFieldsAndMethods().stream().filter(fm -> fm.getType() == Type.METHOD)
				.map(fm -> (Method) fm).filter(m -> m.getName().equals(methodCallExpr.getNameAsString()))
		...
		for (Method m : methods) {
			...
			final Set<String> parameterTypesFromMethod = m.getParameterList().stream().map(ParameterDefinition::getType)
			...
			methodCallExpr.getArguments().forEach(e -> {
				...
				if (e.isNameExpr()) {
					...
					System.out.println(
							"Expression " + e.toString() + " is currently not supported as parameter of method.");
				}
			});

			// check if we have a match with this method
			System.out.println("Checking parameter types for method" + m.toString());
			System.out.println("Comparing " + parameterTypesFromMethod.toString() + " with "
					+ parameterTypesFromStatement.toString());
	 ...
	 * @return
	 */
	public Method getMethodByNameAndParameterSize(final String methodName, List<ParameterDefinition> parameters) { // ,
																													// Map<String,
																													// IFbCReferenceEntity>
																													// diagramVariables)
																													// {
		System.out.println("Searching method with name '" + methodName + "' and the following parameters " + parameters
				+ " in class " + this.getClassName());
		final List<Method> methods = getFieldsAndMethods().stream().filter(fm -> fm.getType() == Type.METHOD)
				.map(fm -> (Method) fm).filter(m -> m.getName().equals(methodName))
				.filter(m -> m.getParameterList().size() == parameters.size()).collect(Collectors.toList());
	...
	public Optional<Field> getFieldForFieldName(String fieldName) {
		return getFieldsAndMethods().stream().filter(fm -> fm.getType() == Type.FIELD).map(fm -> (Field) fm)
