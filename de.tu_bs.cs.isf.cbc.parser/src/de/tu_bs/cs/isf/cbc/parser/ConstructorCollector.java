...
class ConstructorCollector extends VoidVisitorAdapter<Map<String, ConstructorDefinition>> {
	...
	public void visit(ConstructorDeclaration cd, Map<String, ConstructorDefinition> constructors) {
		...
		for (Parameter p : parameterDefintion) {
			...
			// System.out.println(
			// "Mutation modifier not present for constructor parameter, ignoring this
			...
			constructorParameters.put(p.getNameAsString(), new ParameterDefinition(p.getNameAsString(), securityLevel,
					MDF.forAnnotationExpress(mutationModifier), p.getTypeAsString()));
		}
		...
		// Check if all parameters of the constructor are in correspondence to the field
		// declaration of this class
		for (FieldDeclaration fd : clazz.getFields()) {
			// Check if this is a static reference -> no need for a parameter in the
			// constructor
			...
			for (VariableDeclarator variable : fd.getVariables()) {
				if (constructorParameters.get(variable.getNameAsString()) == null) {
					System.out.println("Parameter with name " + variable.getNameAsString()
							+ " is not present in constrcutor definition, ignoring this constrcutor. (Constructor Declaration: "
							+ cd.toString() + ")\n");
					return;
				}
				...
				if (!variable.getTypeAsString().equals(parameterDefinition.getType())) {
					System.out.println("Parameter with name " + variable.getNameAsString()
							+ " does not have the same type as the field with the same name, ignoring this constructor. (Constructor Declaration: "
							+ cd.toString() + ")\n");
					return;
				}
			}
		...
		constructors.put(className,
				new ConstructorDefinition(new ArrayList<ParameterDefinition>(constructorParameters.values())));
	}
}