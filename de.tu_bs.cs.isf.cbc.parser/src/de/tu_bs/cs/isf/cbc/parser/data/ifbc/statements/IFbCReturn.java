...
public class IFbCReturn implements IFbCStatement {
	...
	public IFbCReturn(final String name,
	                  final IFbCStatement returnStatement) {
		this.name = name;
	...
	public TypeableResult isTypeable(final Lattice lattice, 
		...
		case MethodCall:
			...
			break;
		case Return:			
			return new TypeableResult(false, "Return statement should not contain return-statement. Not typeable", elevatedEntities, changedTypes, usedCapsules);
		case Reference:
			...
			break;
		default:
			return new TypeableResult(false,"ERROR: Unsupported type in return statement.", elevatedEntities, changedTypes, usedCapsules);
		...
		if (!MDF.isSubtypeOf(statementMDF, definedMethodMDF)) {
			return new TypeableResult(
					false,
					"Returned MDF " + statementMDF + " is not equal to defined MDF " + definedMethodMDF + " of method " + constructingMethod.getName() +".",
					elevatedEntities, 
					changedTypes, 
					usedCapsules);
		}
		...
		// defined SL of method about to be constructed equal to the SL of the returned object?
		if (statementSL.equals(definedMethodSL)) {
			return new TypeableResult(
					true, 
					"Return is typeable since defined method return type SL (" + definedMethodSL + ") for method " + constructingMethod.getName() + "() and SL of returned object  (" + statementSL + ") is equal.", 
					elevatedEntities, 
					changedTypes, 
					usedCapsules);
		}
		...
		if (LeastUpperBound.secondHigherThanFirst(statementSLNode, definedMethodSLNode)) {				
			...
			if (statementMDF.equals(MDF.IMMUTABLE) | statementMDF.equals(MDF.CAPSULE)) {
				return new TypeableResult(
						true, 
						"Return is typeable because SEC-PROM of returned object (SL: " + statementSL + ", MDF: " + statementMDF + ")  is possible. ", 
						elevatedEntities, 
						changedTypes, 
						usedCapsules);
			}
			...
			return new TypeableResult(
					false, 
					"Return is not typeable because SEC-PROM of returned object (SL: " + statementSL + ", MDF: " + statementMDF + ")  is not possible. ", 
					elevatedEntities, 
					changedTypes, 
					usedCapsules);
		} else {
