...
public abstract class IFbcAbstractStatement implements IFbCStatement {
	...
	@Override
	public abstract TypeableResult isTypeable(Lattice lattice, String projectName,
			Map<String, List<IFbCReferenceEntity>> elevatedEntities, Map<String, String> changedTypes,
			List<String> usedCapsules, String optionalGuardSL, Method constructingMethod) throws IFbCException;
	...
	/**
	 * Check if a provided parsed MDF (from a diagram) is a subtype of a provided
	 * defined subtype (from a code file)
	 * 
	 * @param elevatedEntities
	 *            Map of elevated entities, in case a entity needs to be elevated
	 * @param changedTypes
	 *            Map of changed entities, in case a entity needs to be changed
	 * @param usedCapsules
	 *            List of used capsules, in case a capsule reference is used
	 * @param parsedParameter
	 *            the parsed parameter for logging info
	 * @param parsedParamMDF
	 *            the parsed parameters MDF
	 * @param definedParamMDF
	 *            the defined parameters MDF
	 * 
	 * @return Returns true if the parsed parameter is a subtype of the defined
	...
	protected TypeableResult checkMDFSubtype(final Map<String, List<IFbCReferenceEntity>> elevatedEntities,
			...
			final ParameterDefinition parsedParameter, final MDF parsedParamMDF, final MDF definedParamMDF) {
		if (definedParamMDF.equals(MDF.CAPSULE) && !parsedParamMDF.equals(MDF.CAPSULE)) {
			return new TypeableResult(false,
					"Defined paramters MDF is CAPSULE, runtime parameter can only be CAPSULE as well, is "
	...
	protected TypeableResult checkParameterDefintion(Lattice lattice, String projectName,
			Map<String, List<IFbCReferenceEntity>> elevatedEntities, Map<String, String> changedTypes,
			List<String> usedCapsules, Map<String, JavaClass> javaClassesForProject, ParameterDefinition parameter)
		...
		if (parameter.getOptionalEntity() != null) {
			...
			if (optionalEntity.getEntityType().equals(EntityType.NEW)) {
				...
				final TypeableResult checkNewEntityResult = checkNewEntity(newEntityParam, javaClassesForProject,
						newEntityParam.getType(), projectName, elevatedEntities, changedTypes, usedCapsules, lattice);
				...
				final TypeableResult checkMethodEntityResult = method.isTypeable(lattice, projectName, elevatedEntities,
						changedTypes, usedCapsules, null, methodEntityParam.getDefinedMethod());
				...
				return new TypeableResult(false,
						"EntityType " + optionalEntity.getEntityType()
								+ " not implemented for parsed parameter of constructor or method.",
						elevatedEntities, changedTypes, usedCapsules);
			}
		} else {
		...
		return new TypeableResult(true, "Parameter check valid", elevatedEntities, changedTypes, usedCapsules,
				parsedParameterSL, parsedParamMDF);
	}

	protected TypeableResult checkNewEntity(IFbCNewEntity newEntity, Map<String, JavaClass> javaClassesForProject,
			String constructorName, String projectName, Map<String, List<IFbCReferenceEntity>> elevatedEntities,
			Map<String, String> changedTypes, List<String> usedCapsules, Lattice lattice) throws IFbCException {
		...
		for (int i = 0; i < parameters.size(); i++) {
			...
			// Workaround for null literals - null pointer are irrelevant for information
			// flow
			...
			if (!JavaClasses.isRuntimeParameterMatchingSignature(parsedParameter.getType(), definedParameter.getType(),
					projectName)) {
				return new TypeableResult(false, "Runtime parameter " + parsedParameter.getType()
						+ " is no subtype of defined parameter " + definedParameter.getType() + ".", elevatedEntities,
			...
			final TypeableResult checkParameterSL = checkParameterDefintion(lattice, projectName, elevatedEntities,
					changedTypes, usedCapsules, javaClassesForProject, parsedParameter);
			...
			if (LeastUpperBound.secondHigherThanFirst(parsedParamSLNode, definedParamSLNode)
				...
				if (!isSLPromotable(parsedParamSLNode, definedParamSLNode)) {
					return new TypeableResult(false,
							"Defined parameter security level (" + parsedParamSLNode.getName()
									+ ") is not reachable from  parsed parameter security level ("
									+ parsedParamSLNode.getName() + "). Cannot locate parameter locally.",
							elevatedEntities, changedTypes, usedCapsules);
				}
				if (parsedParamMDF.equals(MDF.MUTABLE) || parsedParamMDF.equals(MDF.READ)) {
					...
					System.out.println("Adding lub(" + parsedParamSLNode.getName() + ", " + definedParamSLNode.getName()
							+ ") to breakingMutAndReadSLs. (Parameters: (defined: " + definedParameter.toString()
							+ ", parsed: " + parsedParameter.toString() + ")");
				} else {
					...
					if (parsedParamMDF.equals(MDF.CAPSULE)) {
						if (usedCapsules.contains(parsedParameter.getName())) {
							return new TypeableResult(false, "Runtime parameter " + parsedParameter.getName()
			...
			final TypeableResult checkMDFSubtype = checkMDFSubtype(elevatedEntities, changedTypes, usedCapsules,
					parsedParameter, parsedParamMDF, definedParamMDF);
		...
		if (breakingMutAndReadSLs.isEmpty()) {
			...
			// lattice.
			System.out.println("All parameters are okay and no SL needs to be determined, using bottom of lattice.");
			...
			if (lub == null) {
				return new TypeableResult(false,
						"No lub() for the following SLs found: " + breakingMutAndReadSLs.toString(), elevatedEntities,
						changedTypes, usedCapsules);
			}
		...
		return new TypeableResult(true, "NewEntity is typeable.", elevatedEntities, changedTypes, usedCapsules, valueSL,
				valueMDF);
	}

	/**
	 * Check if a security level is promotable to another security level, i.e. if
	 * there reachable going upwards from the starting security level.
	 * 
	 * @return
	 */
