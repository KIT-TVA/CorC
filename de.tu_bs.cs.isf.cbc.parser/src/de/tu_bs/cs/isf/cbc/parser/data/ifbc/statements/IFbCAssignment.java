...
public class IFbCAssignment extends IFbcAbstractStatement {
	...
	public TypeableResult isTypeable(final Lattice lattice, final String projectName,
		...
		/**
		 * Check optional guard security level from a potential selection containing
		 * this statement
		 * 
		 * If optionalGuardSL is != null, then this method was called in the context of
		 * a selection.
		 * 
		 * 1) TODO: If guardSL is == bottom of the lattice (meaning the lowest SL), both
		 * branches have to have the same SL. Possibly return the SL of the branch here
		 * in the TypeableResult?
		 *
		 * 2) If the guard SL is > bottom of the lattice, we restrict mutation
		 * (=assignment) in two ways: a) if the target is a non-local variable (meaning
		 * defined in a class, not in the diagram), an assignment to mutable targets is
		 * only possible if the security level of the target is >= guardSL
		 * 
		 * b) if the target is a local variable (meaning defined in the diagram), an
		 * assignment is only possible if the target SL is >= guardSL, disregarding the
		 * type modifier of the target
		 */
		if (optionalGuardSL != null) {
			final Node guardSLNode = lattice.getNodePerName(optionalGuardSL);
			if (guardSLNode == null) {
				return new TypeableResult(false, "Cannot find SL node in lattice for guard: " + optionalGuardSL,
						elevatedEntities, changedTypes, usedCapsules);
			}
			if (optionalGuardSL.equals(lattice.getBottom().getName())) {
				// Case 1, possibly just set a boolean to return the complete SL of the
				// assigment later on
				// TODO: implement
			} else {
				// Case 2
				if (target.getEntityType().equals(EntityType.FIELD_ASSIGN)) {
					...
					if (targetSLNode == null) {
						return new TypeableResult(false, "Cannot find SL node in lattice for guard: " + targetSL,
								elevatedEntities, changedTypes, usedCapsules);
					}

					if (!targetSL.equals(optionalGuardSL)
							&& !LeastUpperBound.secondHigherThanFirst(guardSLNode, targetSLNode)) {
						// try sec-prom
						if (targetMDF.equals(MDF.CAPSULE) || targetMDF.equals(MDF.IMMUTABLE)) {
							...
							if (targetMDF.equals(MDF.CAPSULE)) {
								if (usedCapsules.contains(target.getName())) {
									return new TypeableResult(false, "Target " + target.getName()
							...
							return new TypeableResult(false,
									"Targets " + target.getName() + " security level cannot be promoted, modifier "
											+ targetMDF.toString() + " is not in IMM|CAPS.",
									elevatedEntities, changedTypes, usedCapsules);
						}
					}
				}
		 ...
		 * values include:
		 * 
		 * - literals, object references, i.e. "e = f" - field access, i.e. "e = f.a" -
		 * field access through method return type, i.e "e = f.getFieldObject().text" -
		 * method return type, i.e. "e = f.getFieldObject()" - constructor, i.e. "e =
		 * new C()" - keyword declassify, i.e. "e = declassify(f)" combination with the
		 * above?
		 */
		...
		if (value.getEntityType().equals(EntityType.METHOD)) {
			...
			if (entity.getType().equals("void")) {
				return new TypeableResult(false, "Method return type cannot be 'void' in assignment.", elevatedEntities,
						changedTypes, usedCapsules);
			}

			IFbCMethod methodStatement = new IFbCMethod(entity.getName(), entity);
			final TypeableResult typeableResult = methodStatement.isTypeable(lattice, projectName, elevatedEntities,
		...
		if (value.getEntityType().equals(EntityType.DECLASSIFY)) {
			...
			if (declassify.getParameterMDF().equals(MDF.READ) || declassify.getParameterMDF().equals(MDF.MUTABLE)) {
				return new TypeableResult(false, "Cannot declassify value because MDF is not IMMUTABLE or CAPSULE, is "
						+ declassify.getParameterMDF() + ".", elevatedEntities, changedTypes, usedCapsules);
			}
			valueSL = targetSL;
		...
		if (value.getEntityType().equals(EntityType.NEW)) {
			...
			final TypeableResult checkNewEntityResult = checkNewEntity(newEntity, javaClassesForProject,
					constructorName, projectName, elevatedEntities, changedTypes, usedCapsules, lattice);
			if (!checkNewEntityResult.isTypeable()) {
				return checkNewEntityResult;
			}

			System.out.println("New entity checked typeable with SL " + checkNewEntityResult.getSecurityLevel()
					+ " and MDF " + checkNewEntityResult.getMutationModifier() + ".");
		...
		if (targetSLNode == null || valueSLNode == null) {
			return new TypeableResult(false,
					"Lattice nodes not present for either " + targetSL + " or " + valueSL + ".", elevatedEntities,
					changedTypes, usedCapsules);
		}
		...
		if (target.getEntityType().equals(EntityType.FIELD_ASSIGN)) {
			...
			for (IFbCReferenceEntity scope : entity.getScopes()) {
				if (!scope.getMutationModifier().equals(MDF.MUTABLE)) {
					...
					return new TypeableResult(false, "Scope  " + scope.getName() + " is not mutable.", elevatedEntities,
							changedTypes, usedCapsules);
				}
			}
		...
		if (LeastUpperBound.secondHigherThanFirst(valueSLNode, targetSLNode)) {
			...
			if (target.getMutationModifier().equals(MDF.IMMUTABLE)
				...
				checkChangedTargetType(targetTypeScope, valueReturnType, changedTypes);
				final TypeableResult checkCapsuleUsage = checkCapsuleUsage(elevatedEntities, changedTypes,
						usedCapsules);
				if (checkCapsuleUsage != null) {
					return checkCapsuleUsage;
				}
				return new TypeableResult(true, "target SL is lower than value SL. (using SEC-PROM)", elevatedEntities,
						changedTypes, usedCapsules);
			}
	...
	private TypeableResult checkCapsuleUsage(final Map<String, List<IFbCReferenceEntity>> elevatedEntities,
			final Map<String, String> changedTypes, final List<String> usedCapsules) {
		// capsule used as value?
		if (value.getMutationModifier().equals(MDF.CAPSULE)) {
			if (usedCapsules.contains(value.getName())) {
				return new TypeableResult(false,
						"Value " + value.getName() + " has the type modifier CAPSULE and was already used before.",
						elevatedEntities, changedTypes, usedCapsules);
			} else {
	...
	// private TypeableResult checkPromotionRegardingSpecification(final Lattice
	// lattice,
	...
	// final Node specificationMaxLevelNode =
	// lattice.getNodePerName(specificationMaxLevel);
	// if (specificationMaxLevelNode == null) {
	// return new TypeableResult(
	// false, "Specification level '" + specificationMaxLevel + "' for reference '"
	// + target.getName() + "' does not exist in Lattice." ,
	// elevatedEntities, changedTypes, usedCapsules);
	// }
	//
	// if (LeastUpperBound.secondHigherThanFirst(specificationMaxLevelNode,
	// valueSLNode)) {
	// return new TypeableResult(
	// false, "Using SEC-PROM to elevate target '" + target.getName() + "' to level
	// '" + valueSL + "' is breaking specification of the reference.",
	// elevatedEntities, changedTypes, usedCapsules);
	// }
	// }
	//
	...
	/**
	 * The value SL of the assignment is not conforming to the target SL, trying
	 * local promotion for the different entity types
	 * 
	 * @param lattice
	 *            The used lattice for this type check
	 * @param elevatedEntities
	 *            map of elevated entities
	 * @param changedTypes
	 *            map of changed types
	 * @param usedCapsules
	 *            list of capsule usage
	 * @param targetSL
	 *            the target SL
	 * @param targetTypeScope
	 *            the target type scope
	 * @param valueReturnType
	 *            the value return type
	 * @param targetSLNode
	 *            target SL node from lattice
	 * @param valueMDF
	 * @param valueSL
	 * @return Returns an object of TypeableResult containing the result of the type
	 *         check
	 * 
	 * @throws IFbCException
	 */
	private TypeableResult tryLocalValuePromotion(final Lattice lattice,
			final Map<String, List<IFbCReferenceEntity>> elevatedEntities, final Map<String, String> changedTypes,
			final List<String> usedCapsules, final String targetSL, final String targetTypeScope,
			final String valueReturnType, final Node targetSLNode, final String valueSL, final MDF valueMDF)
			throws IFbCException {
		...
		switch (value.getEntityType()) {
			case ENTITY : // just try to promote locally based on parsed modifier
				if (value.getMutationModifier().equals(MDF.IMMUTABLE)
						|| value.getMutationModifier().equals(MDF.CAPSULE)) {
					// Capsule usage?
					if (value.getMutationModifier().equals(MDF.CAPSULE)) {
						if (usedCapsules.contains(value.getName())) {
							return new TypeableResult(false, "Value " + value.getName()
									+ " has the type modifier CAPSULE and was already used before. Cannot promote locally.",
									elevatedEntities, changedTypes, usedCapsules);
						} else {
							usedCapsules.add(value.getName());
			...
			case FIELD_ACCESS :
				...
				if (fieldAccess.getCombinedMDF().equals(MDF.IMMUTABLE)
						|| fieldAccess.getCombinedMDF().equals(MDF.CAPSULE)) {
					return new TypeableResult(true,
							"Local promotion of field access to target SL " + targetSL
									+ " was possible because combined MDF for expression is "
									+ fieldAccess.getCombinedMDF() + ".",
							elevatedEntities, changedTypes, usedCapsules);
				}
				...
				for (int i = 0; i < scopeSecurityLevels.size(); i++) {
					...
					return new TypeableResult(false, "Cannot use local promotion for scope "
							+ fieldAccess.getScopes().get(i) + " of field access value.", elevatedEntities,
							changedTypes, usedCapsules);
				}

				// all checks in the for-loop where successful -> we can elevate all scopes
				return new TypeableResult(true,
						"Local promotion of field access to target SL " + targetSL + " was possible.", elevatedEntities,
						changedTypes, usedCapsules);
			case METHOD :
				// Elevation possible or is only the defined SL for the return type possible?
				// depending on defined MDF?
				if (valueMDF.equals(MDF.IMMUTABLE) || (valueMDF.equals(MDF.CAPSULE))) {
					return new TypeableResult(true,
							"Local promotion of method return object to target SL " + targetSL
									+ " was possible because method MDF for expression is " + valueMDF + ".",
							elevatedEntities, changedTypes, usedCapsules);
				} else {
					return new TypeableResult(false,
							"Local promotion of method return object to target SL " + targetSL
									+ " was not possible because method MDF for expression is " + valueMDF + ".",
							elevatedEntities, changedTypes, usedCapsules);
				}
			case NEW :
				// Shoudn't this be done by the developer only through using corresponding
				// parameters for the constructor?
				return new TypeableResult(false, "Unsupported local promotion. (NEW)", elevatedEntities, changedTypes,
						usedCapsules);
			default :
				return new TypeableResult(false, "Unsupported local promotion. (" + value.getEntityType() + ")",
						elevatedEntities, changedTypes, usedCapsules);
		}
	}

	private void checkChangedTargetType(String targetTypeScope, String valueReturnType,
			final Map<String, String> changedTypes) {
		System.out.println(
				"Checking if target type " + targetTypeScope + " differs from value type " + valueReturnType + ".");
